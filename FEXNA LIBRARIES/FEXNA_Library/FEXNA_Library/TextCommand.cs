...
namespace FEXNA_Library
{
    public class TextCommand
    {
        ...
        public readonly static Dictionary<string, TextCommand> COMMANDS = new Dictionary<string, TextCommand>
        {
            ...
            { "S",      new TextCommand { Identifier = (char)0x04, Tag = @"\\[Ss]", RegexStr = @"[-]{0,1}\w+", StartupCommand = true, NewlineAfterFirst = true } },
            ...
            // \X: Pitch change
            { "X",      new TextCommand { Identifier = (char)0x10, Tag = @"\\[Xx]", RegexStr = @"[0-9]+" } },
            // \scroll: Force scroll
        ...
        private string regex_string { get { return null_replacement ? "" : string.Format(@"\[({0})\]", RegexStr); } }
