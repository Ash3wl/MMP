                    ...
                    switch (New_Turn_Phase)
                    {
                        case 0:
                            ...
                            foreach (int id in New_Turn_Team)
                            {
                                ...
                                Global.game_map.units[id].new_turn_cantoforce();
                            }
        ...
        protected bool update_new_turn_terrain()
        {
            ...
            switch (New_Turn_Action)
            {
                case 0:
                    ...
                    if (new_turn_unit.actor.attack_stacks > 0)
                        new_turn_unit.actor.attack_stacks--;
                    ...
                    if (new_turn_unit.move_cost(new_turn_unit.loc) == -1)
                    {
                        ...
                        if (terrain.Name != "--" && terrain.Name != "Village" && terrain.Id != 68)
                        {
                            New_Turn_Healing_Value += -(int)Math.Ceiling(new_turn_unit.actor.maxhp * 0.25f);
                ...
                case 5:
                    ...
                    {
                        case 0:
                            new_turn_unit.actor.hp += New_Turn_Healing_Value;
                            if (Constants.Map.RESCUED_TERRAIN_HEAL && New_Turn_Terrain_Healing_Value > 0)
                                new_turn_unit.rescuing_unit.actor.hp += New_Turn_Terrain_Healing_Value;
                            if (New_Turn_Healing_Value < 0)
                            {
