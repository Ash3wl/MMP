...
namespace FEXNA
{
    partial class Game_State
    {
            ...
            if (Global.Input.triggered(Inputs.B))
            {
                     ...
                     !Global.game_map.scrolling)
                {
                    if (Global.game_temp.highlighted_unit_id == -1 ||
                        !highlighted_unit.is_attackable_team(Constants.Team.PLAYER_TEAM))
                    {
        ...
        public string get_death_quote(int id)
        {
            ...
            if (Death_Quotes.ContainsKey(Global.game_map.units[id].actor.id) && Global.death_quotes.ContainsKey(Death_Quotes[Global.game_map.units[id].actor.id]))
            {
        ...
        // Battle Themes
        public void remove_battle_theme(int id, string value)
        {
            Unit_Battle_Themes.Remove(id);
        }
        ...
        public Game_Ranking calculate_ranking_progress()
        {
            ...
            int enemies_remaining = Global.game_map.units.Values.Count(x => x.is_attackable_team(Constants.Team.PLAYER_TEAM));
        ...
        public void play_turn_theme(int volume = 100, bool fade_in = false, int teamTurn = -1)
        {
            ...
            if (teamTurn == Constants.Team.PLAYER_TEAM && near_victory())
                Turn_Theme = Config.VICTORY_THEME;
        ...
        public void play_turn_theme(string name, bool force_restart_theme = false)
        {
            ...
            if (string.IsNullOrEmpty(Turn_Theme))
                return;
            if (Global.game_system.SWITCHES[99])
            {
                Global.Audio.play_map_bgm("HandfulOfHope", false, force_restart_theme);
                return;
            }
            Global.Audio.play_map_bgm(Turn_Theme, false, force_restart_theme); //Yeti
        }
        ...
        public void resume_turn_theme(bool fade)
        {
            if (Global.game_system.SWITCHES[99])
            {
                Global.Audio.play_map_bgm("HandfulOfHope", false);
                return;
            }
            if (!Global.game_system.preparations && !Global.game_system.is_victory()) //Yeti
                Global.Audio.play_map_bgm(Turn_Theme, fade);
        }
        ...
        public void play_staff_theme()
        {
            ...
            Global.Audio.play_bgm(Config.STAFF_THEME);
        }
        ...
        public void play_attack_staff_theme()
        {
            ...
            Global.Audio.play_bgm(Config.ATTACK_STAFF_THEME);
        }
        ...
        public void play_dance_theme()
        {
            ...
            Global.Audio.play_bgm("Poem of Breeze"); //Yeti
        }

        public void play_promotion_theme()
        {
            Global.Audio.play_bgm(Config.PROMOTION_THEME);
        }

        public void play_preparations_theme()
        {
            if(Global.game_system.SWITCHES[97])
                Global.Audio.play_bgm("WagnerianFinale");
            else
                Global.Audio.play_bgm(Config.PREPARATIONS_THEME);
        }
        #endregion
        ...
        public bool no_highlight()
        {
            return combat_active ||
                is_changing_turns || visit_active || Global.game_map.units_waiting() || support_active || talk_active ||
                item_active || dance_active || steal_active || block_active || Global.game_map.changing_formation || no_cursor;
        }
