...
{
    public enum Rescue_Modes { None, Rescue, Drop, Take, Give, Cover }
    class Game_Rescue_State : Game_State_Component
    {
        ...
        protected Rescue_Modes rescue_mode
        {
            get
            {
                if (In_Rescue == Rescue_Modes.Cover) return Rescue_Modes.Rescue;
                return In_Rescue;
            }
        }
        ...
        private IEnumerable<bool> cleanup_rescue()
        {
            ...
            {
                if (rescuer.is_active_player_team && !rescuer.berserk) //Multi
                {
                    // Adds horse canto
                    Global.game_system.Menu_Canto |= Canto_Records.Horse;
                    rescuer.open_move_range();
                }
            }
            else
            {
                rescuer.start_wait(false);
                // Cancels canto state
