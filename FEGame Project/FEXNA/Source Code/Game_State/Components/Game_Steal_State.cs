...
namespace FEXNA.State
{
    class Game_Steal_State : Game_State_Component
    {
        ...
        private int steal_chance(Game_Actor stealer, Game_Actor stealee, int value)
        {
            int rate;
            rate = (2000 - value) / 10;
            int level_bonus = (stealer.level - stealee.level) * 5;
            rate = (int)MathHelper.Clamp(rate - level_bonus, 0, 100);
            return 0;
            
        }

        private bool roll_for_steal(Game_Actor stealer, Game_Actor stealee, int value)
        {
            if (Global.game_system.get_rng() < steal_chance(stealer, stealee, value))
                return true;
            return false;
        }

        private void vandy_swear()
        {
            int clip_num = (int)Math.Round(Global.game_system.get_rng() / 10f);
            string clip = "VandyFail" + clip_num.ToString();
            Global.Audio.play_se("Battle Sounds", clip);
        }
                    ...
                    switch (Steal_Phase)
                    {
                        ...
                        case 2:
                            ...
                            { 
                                case 0:
                                    //SUCCESS OR FAIL?
                                    if (roll_for_steal(stealer.actor, steal_target.actor, steal_target.actor.read_item(Stolen_Item).cost))
                                    {
                                        vandy_swear();
                                        get_scene_map().set_item_stolen_popup(null, 113);
                                    }
                                    else
                                    {
                                        ...
                                        stealer.actor.gain_item(stolen_item); //wark vandy steal processing here
                                        if (stealer.is_attackable_team(Constants.Team.PLAYER_TEAM))
                                        {
                        ...
                        case 3:
                            switch (Steal_Action)
                            {
                                case 0:
                                    ...
                                    {
                                        case 0:
                                            if (stealer.is_ally && stealer.actor.can_level())
                                            {
                                                Global.game_state.exp_gauge_gain = (Constants.Combat.STEAL_EXP > 0 ?
                                                ...
                                                stealer.actor.exp += Constants.Combat.STEAL_EXP;
                                                Steal_Timer++;
                                            }
                                            else
                                                Steal_Phase++;
