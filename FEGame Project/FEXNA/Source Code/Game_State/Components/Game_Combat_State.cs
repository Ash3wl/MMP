...
namespace FEXNA.State
{
    ...
    partial class Game_Combat_State : Game_Combat_State_Component
    {
        ...
        internal string battle_theme
        {
            get
            {
                ...
                if (Global.game_system.In_Arena)
                    filename = Config.ARENA_BATTLE_THEME;
        ...
        public Game_Unit enemy_of_dying_unit()
        {
            if (!In_Battle)
            {
                ...
                if (Units.ContainsKey(Global.game_temp.dying_unit_id) &&
                        Units[Global.game_temp.dying_unit_id].is_attackable_team(Constants.Team.PLAYER_TEAM) &&
                            ...
                            case 5:
                                if (is_target_unit)
                                    get_scene_map().check_talk(Battler_1, Battler_2,
                                        !Battler_1.is_attackable_team(Constants.Team.PLAYER_TEAM));
                                Combat_Timer++;
                                break;
                            case 6:
                            case 7:
        ...
        protected bool update_cleanup_actions(Game_Unit Battler_1, Game_Unit Battler_2)
        {
            switch (Cleanup_Action[0][0])
            {
                case (int)Cleanup_Actions.Promotion:
                    ...
                    {
                        case 0:
                            Global.Audio.play_se("System Sounds", "Level_Up_Fanfare");
                        ...
                        case 4:
                            Global.game_system.Battle_Mode = Global.game_options.animation_mode ==
                                (byte)Animation_Modes.Map ? Animation_Modes.Map : Animation_Modes.Full; //Yeti
        ...
        {
            if (Units[Battler_1_Id].continue_attacking()) return true;
            if (Units.ContainsKey(Battler_2_Id) && Units[Battler_2_Id].continue_attacking()) return true;
            if (In_Aoe)
                    ...
                    switch (Battle_Action[0])
                    {
                        ...
                        case (int)Map_Battle_Actions.Attack:
                            ...
                            if (get_scene_map().is_map_effect_hit())
                            {
                                ...
                                // Else if no damage
                                //else if ((dmg == 0 && !battler_1.actor.astra_activated) && !Map_Combat_Data.Data[Attack_Id].Key.Result.kill) //warkastra maybe
                        ...
                        case (int)Map_Battle_Actions.Wait_After_Attack:
                            ...
                            if (!get_scene_map().is_map_effect_active() && (Combat_Timer > 23 || continue_attacking(true)))
                            {
                                ...
                                Combat_Timer = 0;
                                if (Map_Combat_Data.Data[Attack_Id].Key.Result.state_change.Count > 0 && !is_battler_dead(ids[1]) &&
                                        !skip_skill_update() && Map_Combat_Data.Data[Attack_Id].Key.Result.status_inflict_map_id() > 0)
                            ...
                            case 2:
                                ...
                                if (Battle_Transition_Timer == 0)
                                {
                                    ...
                                    if (Skipping_Battle_Scene)
                                    {
                                        ...
                                        Global.game_system.Battle_Mode = Animation_Modes.Map;
                                        Map_Battle = true;
                                        Combat_Phase = 2;
                                        Combat_Action = 1;
                                        Map_Combat_Data.apply_combat();
        ...
        protected void setup_battle()
        {
            ...
            set_animation_mode(Global.game_system.Battler_1_Id, Global.game_system.Battler_2_Id, fighting, Map_Combat_Data);
            Map_Combat_Data = null;
            Map_Battle = Global.game_system.Battle_Mode == Animation_Modes.Map;
        }
