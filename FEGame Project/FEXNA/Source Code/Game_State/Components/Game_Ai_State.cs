...
namespace FEXNA.State
{
    ...
    class Game_Ai_State : Game_State_Component
    {
                ...
                case 31:
                    switch (Ai_Action)
                    {
                        ...
                        case Ai_Actions.Idle:
                            ...
                            else
                            {
                                ...
                                if (unit.mission != Game_AI.RETREATING_ATTACK_MISSION &&
                                        !unit.uncontrollable && Global.game_map.seize_points[unit.team].Count > 0)
                                    if (Ai_Move_Range.Intersect(Global.game_map.seize_points[unit.team]).Any())
                                    {
                                ...
                                if (Useable_Weapons.Count > 0 && !unit.berserk && unit.is_rescuing && Ai_Enemy_Attack_Range.Contains(unit.loc))
                        ...
                        case Ai_Actions.Seize:
                            ...
                            if (Global.game_map.seize_points[unit.team].Count > 0)
                        ...
                        case Ai_Actions.Search_For_Targets:
                            search_loc = null;
                            bool seizing = false;
                            // If seize points are a thing, move toward those instead of enemies
                            if (!unit.uncontrollable && Global.game_map.seize_points[unit.team].Count > 0 && !Defending_Area)
                        ...
                        case Ai_Actions.Finish_Movement:
                            if (!unit.is_in_motion())
                            {
                                if (!unit.cantoing)
                                {
                                    if (Global.game_map.seize_points[unit.team].Contains(unit.loc))
                    ...
                    switch (Ai_Action)
                    {
                        ...
                        case Ai_Actions.Idle:
                            ...
                            else
                            {
                                ...
                                pillage_targets = ai_pillage_targets(unit);
                ...
                case 15:
                    switch (Ai_Action)
                    {
                        ...
                        case Ai_Actions.Idle:
                            ...
                            if (!Global.game_map.team_escape_point_locations(unit.team, unit.group).Any())
                            {
                                unit.mission = Game_AI.ATTACK_IN_RANGE_MISSION;
                            ...
                            else
                            {
                                ...
                                HashSet<Vector2> escape_locations = Global.game_map.team_escape_point_locations(unit.team, unit.group);
                        ...
                        case Ai_Actions.Move_Wait:
                            switch (Ai_Timer)
                            {
                                case 11:
                                    Ai_Timer = 0;
                                    unit.evented_move_to(Global.game_map.team_escape_point_exit(unit, Temp_Ai_Loc));
                        ...
                        case Ai_Actions.Wait_For_Move_Escape:
                            if (unit.loc == Global.game_map.team_escape_point_exit(unit, Temp_Ai_Loc))
                    ...
                    switch (Ai_Action)
                    {
                        ...
                        case Ai_Actions.Idle:
                            ...
                            else
                            {
                                ...
                                foreach (int id in unit.check_range(1, 1, Ai_Move_Range, attackable: false)[0])
                    ...
                    switch (Ai_Action)
                    {
                            ...
                            else
                            {
                                ...
                                if (!(unit.actor.staff_fix() || unit.actor.weapon == null))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (unit.is_attackable_team(Constants.Team.PLAYER_TEAM))
                                        {
                                            unit_index = 0;
                        ...
                        case Ai_Actions.Selecting_Target:
                            ...
                            else
                            {
                                ...
                                Temp_Ai_Target = Game_AI.get_en_route_atk_target(unit, enemies_en_route, useable_weapons, true, false);
                                if (Temp_Ai_Target == null)
                                    Ai_Action = Ai_Actions.Search_For_Targets;
                                else
                                {
                    ...
                    switch (Ai_Action)
                    {
                        ...
                        case Ai_Actions.Idle:
                            ...
                            else
                            {
                                ...
                                foreach (int id in unit.check_range(1, 1, Ai_Move_Range)[0])
                                ...
                                case 1:
                                    if (!Scrolling || Skip_Ai_Turn)
                                    {
                                        ...
                                        if (trade_target.IsSomething)
                                        {
                                            ...
                                            if (!Skip_Ai_Turn)
                                                get_scene_map().set_item_received_popup(Units[trade_target_id].actor.items[trade_item_index], 60);

                                            unit.actor.items[unit.actor.num_items] = Units[trade_target_id].actor.items[trade_item_index];
        ...
        private bool attack_target(Game_Unit unit)
        {
            if (!Global.player.is_targeting())
            {
                ...
                unit.using_siege_engine =
                    Temp_Ai_Target[1] - 1 >= Constants.Actor.NUM_ITEMS;
                // Unit movement locked in
                unit.moved();
                Game_Unit target = Units[Temp_Ai_Target[0]];
        ...
        private List<LocationDistance> ai_pillage_targets(Game_Unit unit)
        {
            ...
            for (int mode = 0; mode <= modes; mode++)
            {
                switch (mode)
                {
                    ...
                    case (int)NextAIUnitModes.Seize:
                        if (Global.game_map.seize_points[Team_Turn].Count <= 0)
                            continue;
                        for (int i = 0; i < ai_team.Count; i++)
                        {
                            ...
                            if (Game_AI.ATTACK_MISSIONS.Contains(ai_mission) &&
                                ...
                                if (update_ai_move_range(unit).Intersect(Global.game_map.seize_points[unit.team]).Any())
                    ...
                    case (int)NextAIUnitModes.Other:
                        for (int i = 0; i < ai_team.Count; i++)
                        {
                            ...
                            // If on attack mission and not rooted in place
                            if (Game_AI.ATTACK_MISSIONS.Contains(ai_mission) &&
                                // Attack in place/attack in move range missions can't self heal? //Debug
