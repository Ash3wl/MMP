...
namespace FEXNA
{
    partial class Game_State
    {
        ...
        {
            if (action == Event_Action_Triggers.Any)
            { }
            HashSet<int> events_to_start = new HashSet<int>();
            // Get events that match the parameters
            int index = 0;
...
#if DEBUG
            throw new ArgumentException(string.Format("Failed to find and run event named\n\"{0}\"", name));   
#endif
            return false;
        }
    ...
    public class Event_Data_Handler
    {
        ...
        {
#if DEBUG
            System.Diagnostics.Debug.Assert(Global.content_exists(EVENTS_PATH + name),
                string.Format("Event data file \"{0}\" does not exist", name));
#endif
            Event_Data.Events.AddRange(Global.Chapter_Text_Content.Load<Map_Event_Data>(EVENTS_PATH + name).Events);
        }
