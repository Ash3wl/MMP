...
namespace FEXNA
{
    partial class Game_Map
    {
        ...
        {
            FEXNA_Library.Data_Terrain terrain = terrain_data(loc);
            return terrain.Move_Costs[Global.game_state.weather][unit.actor.move_type]; // This might need to be a Game_Unit method //Yeti
        }
        internal int terrain_cost(FEXNA_Library.MovementTypes move_type, Vector2 loc)
        {
            return terrain_cost((int)move_type, loc);
        }
        internal int terrain_cost(int move_type, Vector2 loc)
        {
            FEXNA_Library.Data_Terrain terrain = terrain_data(loc);
            return terrain.Move_Costs[Global.game_state.weather][move_type];
        }
