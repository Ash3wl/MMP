...
namespace FEXNA
{
    partial class Game_Map
    {
        ...
        public bool is_blocked(Vector2 loc, int id, bool fow)
        {
            ...
            if (unit != null && fow && !unit.visible_by(Units[id].team))
                unit = null;
        ...
        public HashSet<Vector2> get_unit_range(HashSet<Vector2> move_range, int min_range, int max_range, bool walls)
        {
            ...
            return Pathfinding.get_range_around(move_range, max_range, min_range, walls);
        }
        ...
        protected bool toggle_enemy_range(int id)
        {
            ...
            {
                if (Units[id].attack_range.Count > 0 || Units[id].staff_range.Count > 0)
                {
        ...
        {
            if (!Units[id].visible_by())
                return;
            foreach (Vector2 loc in Units[id].attack_range)
                if (!attack.ContainsKey(loc))
                    attack.Add(loc, 0);
            foreach (Vector2 loc in Units[id].staff_range)
        ...
        public bool is_enemy_range_visible()
        {
            ...
            if (Global.scene.is_message_window_active || Global.game_system.is_interpreter_running) return false;
            return true;
        }
        #endregion
        ...
        public void update_move_arrow()
        {
            ...
            {
                int range = Units[Global.game_system.Selected_Unit_Id].canto_mov;
                // If the cursor is in the move range
                if (Move_Range.Contains(Global.player.loc))
                {
                    if (Global.player.loc == Units[Global.game_system.Selected_Unit_Id].loc)
                    {
                        Move_Arrow.Clear();
                    }
                    else
                    {
                        ...
                        if (Move_Arrow.Count > 0)
                        {
                            ...
                            if (already_on_list)
                            {
                                ...
                                while (j >= i)
                                {
                                    ...
                                    Selected_Move_Total -= Units[Global.game_system.Selected_Unit_Id].move_cost(new Vector2(loc.X, loc.Y));
                        ...
                        if (!adjacent)
                        { }
                        Selected_Move_Total += Units[Global.game_system.Selected_Unit_Id].move_cost(Global.player.loc);
        ...
        protected void reset_move_arrow(Vector2 target_loc, int range)
        {
            ...
            Move_Arrow.Clear();
            List<Vector2> route2 = Pathfinding.get_route(
                target_loc, range, Global.game_system.Selected_Unit_Id);
            List<Vector2> route = Pathfinding.get_reverse_route(
                target_loc, range, Global.game_system.Selected_Unit_Id);
            Vector2 current_loc = Units[Global.game_system.Selected_Unit_Id].loc;
            ...
            while (route.Count > 0)
            {
                ...
                current_loc += dir;
                Selected_Move_Total += Units[Global.game_system.Selected_Unit_Id].move_cost(current_loc);
