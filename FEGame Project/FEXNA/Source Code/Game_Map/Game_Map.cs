...
namespace FEXNA
{
    internal partial class Game_Map
    {
        ...
        private Dictionary<string, int> Unit_Identifiers = new Dictionary<string, int>();
        private Dictionary<int, Destroyable_Object> Destroyable_Objects = new Dictionary<int, Destroyable_Object>();
        private Dictionary<int, Siege_Engine> Siege_Engines = new Dictionary<int, Siege_Engine>();
        ...
        private List<Team_Escape_Data> Team_Escape_Points;
        private HashSet<Vector2>[] Seize_Points = new HashSet<Vector2>[Constants.Team.NUM_TEAMS + 1];
        ...
        public void write(BinaryWriter writer)
        {
            ...
            Units.write(writer);
            Unit_Identifiers.write(writer);
            Destroyable_Objects.write(writer);
            Siege_Engines.write(writer);
            ...
            Team_Escape_Points.write(writer);
        ...
        public void read(BinaryReader reader)
        {
            ...
            Units.read(reader);
            Unit_Identifiers.read(reader);
            Destroyable_Objects.read(reader);
            Siege_Engines.read(reader);
            fix_siege_locations();
        ...
        public void load_suspend()
        {
            ...
            foreach (Game_Unit unit in Units.Values)
                unit.init_sprites();
            foreach (Siege_Engine siege in Siege_Engines.Values)
                siege.init_sprites();
            update_enemy_range();
        ...
        public bool active_team_turn_over
        {
            get
            {
                return !Teams[team_turn].Any(id =>
                        {
                            if (!Units[id].ready ||
                                    Units[id].is_rescued ||
                                    is_off_map(Units[id].loc) ||
                                    Units[id].uncontrollable ||
                                    Units[id].unselectable)
                                return false;
                            return Global.game_options.auto_turn_end == 2 ? Units[id].mov > 0 : true;
                        });
            }
        }

        internal Dictionary<int, Game_Unit> units { get { return Units; } }
        internal Dictionary<int, Destroyable_Object> destroyable_objects { get { return Destroyable_Objects; } }
        internal Dictionary<int, Siege_Engine> siege_engines { get { return Siege_Engines; } }

        public Dictionary<string, int> unit_identifiers { get { return Unit_Identifiers; } }
        ...
        {
            get { return Move_Range_Visible && Waiting_Units.Count == 0 && !Global.game_system.is_interpreter_running; }
            set
            {
        ...
        public HashSet<Vector2>[] seize_points { get { return Seize_Points; } }
        ...
        public Game_Map()
        {
            ...
            Team_Escape_Points = new List<Team_Escape_Data>();
            ...
            for (int i = 0; i <= Constants.Team.NUM_TEAMS; i++)
            {
                ...
                Team_Escape_Points.Add(new Team_Escape_Data());
                Team_Leaders[i] = -1;
        ...
        {
            Unit_Data = Unit_Data_Name == "" ? new Map_Unit_Data() : Global.Chapter_Text_Content.Load<Map_Unit_Data>(@"Data/Map Data/Unit Data/" + Unit_Data_Name);
        }
        ...
        internal void setup(string id, Data_Map map_data)
        {
            ...
            Units.Clear();
            Unit_Identifiers.Clear();
            Destroyable_Objects.Clear();
            Siege_Engines.Clear();
        ...
        protected void debug_map_stuff()
        {
            ...
            if (Units[Last_Added_Unit_Id].actor.level == 10)
            {
                Units[Last_Added_Unit_Id].actor.gain_item(new Item_Data(Item_Data_Type.Weapon, 1, -1));
                Units[Last_Added_Unit_Id].actor.instant_level = true;
                Units[Last_Added_Unit_Id].actor.exp += 1000;
                Units[Last_Added_Unit_Id].actor.gain_item(new Item_Data(Item_Data_Type.Weapon, 12, 25));
                Units[Last_Added_Unit_Id].actor.gain_item(new Item_Data(Item_Data_Type.Item, 15, 5));
                        ...
                        foreach(Generic_Builds build in builds)
                            foreach (int class_id in class_ids)
                            {
                                ...
                                foreach (int enemy_id in enemy_ids[level][(int)build])
                                {
                                    ...
                                    Game_Unit enemy = Units[enemy_id];
        ...
        internal Game_Unit get_unit(Vector2 loc)
        {
            ...
            if (id == -1)
                return null;
            if (Units[id].loc != loc && !Units[id].saving_ai_loc &&
        ...
        {
            if (Unit_Identifiers.ContainsKey(identifier))
                return Unit_Identifiers[identifier];
            return -1;
        }
        ...
        internal Siege_Engine get_siege(Vector2 loc)
        {
            ...
            return Siege_Engines[id];
        }

        protected void fix_destroyable_locations()
        {
            Destroyable_Locations = new int[this.width, this.height];
            foreach (Destroyable_Object map_object in Destroyable_Objects.Values)
        ...
        public Destroyable_Object get_destroyable(Vector2 loc)
        {
            ...
            if (id == -1) return null;
            if (Destroyable_Objects[id].loc != loc)
            {
#if DEBUG
        ...
        internal Game_Unit get_unit_from_actor(int actor_id)
        {
            ...
            return Units.ContainsKey(unit_id) ? Units[unit_id] : null;
        }
        ...
        {
            if (Global.game_temp.highlighted_unit_id != -1 && Units.ContainsKey(Global.game_temp.highlighted_unit_id))
                return units[Global.game_temp.highlighted_unit_id];
            return null;
        }
        ...
        internal Game_Unit get_selected_unit()
        {
            if (Global.game_system.Selected_Unit_Id != -1)
                return units[Global.game_system.Selected_Unit_Id];
            return null;
        }
        ...
        internal void kill_unit(Game_Unit unit, bool dead = true)
        {
            ...
            if (dead)
            {
                ...
                if (unit.is_ally && (!unit.is_convoy() ||
                    Global.game_system.loss_on_death.Contains(unit.actor.id) || Global.game_system.ally_loss_on_death.Contains(unit.actor.id)))
                {
                    unit.actor.send_ring();
        ...
        internal void add_unit_move_range_update(Game_Unit unit, Vector2 loc) //private //Yeti 
        {
            ...
            for (int team = 1; team <= Constants.Team.NUM_TEAMS; team++)
            {
                ...
                Team_Range_Updates[team].Add(unit.turn_start_loc);
                // This shouldn't be needed unless a unit somehow ends up in the same tile as another unit??? //Yeti
                if (unit.loc != unit.turn_start_loc)
                    foreach (Game_Unit other_unit in Units.Values)
                        if (other_unit.loc == unit.turn_start_loc)
                        {
                            other_unit.fix_unit_location(true);
                            break;
                        }
                //Team_Range_Updates[team] = Team_Range_Updates[team].Distinct().ToList(); //ListOrEquals //HashSet
            }
        }
        ...
        public int highest_priority_unit(int team_id, bool boss = false)
        {
            ...
            if (Units.ContainsKey(Team_Leaders[team_id]) && Units[Team_Leaders[team_id]].visible_by())
                return Team_Leaders[team_id];
            // Search through bosses
            int unit_priority = -1;
            int unit_id = -1;
            foreach (int id in Units.Keys)
                if (Units[id].boss && Units[id].visible_by() && Units[id].team == team_id) // This should require the unit be on the given team, right...? //Yeti
                    if (Units[id].priority > unit_priority)
                    {
                        unit_id = id;
                        unit_priority = Units[id].priority;
                    }
            if (boss || unit_id > -1)
                return unit_id;
            ...
            unit_id = -1;
            foreach (int id in Units.Keys)
                if (Units[id].visible_by() && Units[id].priority > unit_priority && Units[id].team == team_id) // This should require the unit be on the given team, right...? //Yeti
                {
                    unit_id = id;
                    unit_priority = Units[id].priority;
                }
        ...
        public void refresh_move_ranges(bool update_all)
        {
            ...
            //foreach (int id in Units.Keys)
            //    ids.Add(id);
            List<int> ids = new List<int>(Units.Keys);
            ...
            foreach (int id in ids)
                if (Units[id].is_dead && !Units[id].dead)
                    Units[id].kill(false);
        }
            ...
            lock (Fow_Visibility)
            {
                ...
                for (int i = 0; i < Constants.Team.TEAM_GROUPS.Length; i++)
                {
                    ...
                    for (j = 0; j < team.Count; j++)
                        viewers.Add(new Fow_View_Object(Units[team[j]].loc, Units[team[j]].vision()));
                    viewers.AddRange(Torch_Staves);
                    HashSet<Vector2> visibility = Pathfinding.fow_sight_area(viewers);
                    foreach (int team_id in group)
            ...
            if (Refresh_Move_Ranges)
            {
                ...
                if (do_refresh)
                {
                    ...
                    foreach (Game_Unit unit in Units.Values)
                    {
                        ...
                        {
#if DEBUG
                             System.Diagnostics.Debug.Assert(unit.rescued == 0, "Unit being removed for being off map\nshouldn't be rescued, but is in the middle\nof rescue code?");
#endif
                            unit.kill(false);
        ...
        protected void update_unit_wait()
        {
            ...
            {
                if (!Units.ContainsKey(Waiting_Units[i].Item1))
            ...
            if (Waiting_Units.Count > 0)
            {
                if (!Global.game_system.is_interpreter_running)
                {
                    ...
                    foreach (Tuple<int, bool> pair in Waiting_Units)
                        Units[pair.Item1].wait(pair.Item2);
                    Waiting_Units.Clear();
                    Waiting_Unit_Skip.Clear();
        ...
        protected void update_units()
        {
            foreach (var unit in Units)
            {
                unit.Value.update();
            }
            foreach (var siege in Siege_Engines)
            {
                siege.Value.update();
            }
        }
        ...
        public void highlight_test()
        {
            ...
            if (Units.Keys.Contains(Global.game_temp.highlighted_unit_id))
            {
                // Test with the unit if it is still highlighted
                Game_Unit highlighted_unit = Units[Global.game_temp.highlighted_unit_id];
                highlighted_unit.highlight_test();
                if (highlighted_unit.highlighted) return;
            }
            ...
            Game_Unit highlighted_unit, bool onlyIfSelectable = false)
        {
            ...
            {
                if (highlighted_unit.is_attackable_team(Constants.Team.PLAYER_TEAM) &&
                        onlyIfSelectable)
                    return false;
        ...
        internal void next_unit(Game_Unit highlighted_unit, bool selected_moving)
        {
            if (!Scrolling && !Global.game_state.is_menuing && Global.game_state.is_map_ready() && !selected_moving)
            {
                ...
                else
                {
                    ...
                    if (Teams[team].Contains(Team_Leaders[team]) && Units[Team_Leaders[team]].ready &&
                            !Units[Team_Leaders[team]].is_rescued && Units[Team_Leaders[team]].visible_by())
                        temp_team.Add(Team_Leaders[team]);
                    foreach (int id in Teams[team])
                        if (Units[id].ready && !Units[id].is_rescued && Units[id].visible_by() && !is_off_map(Units[id].loc) && !temp_team.Contains(id))
                            temp_team.Add(id);
                }
                if (temp_team.Count > 0)
                {
                    ...
                    if (temp_team.Contains(Global.game_temp.highlighted_unit_id))
                    {
                        ...
                        {
                            Units[Global.game_temp.highlighted_unit_id].highlighted = false;
                    ...
                    if (moved)
                    {
                        ...
                        highlighted_unit = Units[Global.game_temp.highlighted_unit_id];
 ...
 || Global.scene.scene_type == "Scene_Map_Unit_Editor"
        ...
        public Vector2 scroll_dist(Vector2 dest_loc)
        {
                ...
                dest_loc.Y >= this.height - y_soft_center) && can_scroll(8)) // Constant for scroll area //Yeti
            {
                ...
                y = Math.Min(dest_loc.Y + y_soft_center + 1, this.height) - ((Config.WINDOW_HEIGHT / TILE_SIZE) - Target_Display_Y);
            }
            float x = 0;
            // Scroll Left
                ...
                dest_loc.X >= this.width - x_soft_center) && can_scroll(6)) // Constant for scroll area //Yeti
            {
                ...
                x = Math.Min(dest_loc.X + x_soft_center + 1, this.width) - ((Config.WINDOW_WIDTH / TILE_SIZE) - Target_Display_X);
            }
        ...
        {
            Team_Escape_Points[team].add_point(group, loc, escape_to_loc);
        }

        internal HashSet<Vector2> team_escape_point_locations(int team, int group)
        {
        ...
        internal Vector2 team_escape_point_exit(Game_Unit unit, Vector2 loc)
        {
        ...
        public void add_seize_point(int team, Vector2 loc)
        {
            //if (!Seize_Points[team].Contains(loc)) // This is a hashset, so contains shouldn't be needed? //Debug
            Seize_Points[team].Add(loc);
        }

        public void seize_point(int team, Vector2 loc)
        {
            if (Seize_Points[team].Contains(loc))
            {
                Seize_Points[team].Remove(loc);
                Seized_Points.Add(loc);
                Global.game_system.VARIABLES[73]++;
            }
        }
        ...
        { 
            ...
            for (int i = 0; i < Seize_Points.Length; i++)
                Seize_Points[i] = new HashSet<Vector2>();
            Seized_Points = new HashSet<Vector2>();
            //wark seize stuff around here
            Global.game_system.VARIABLES[73] = 0;
        }
        ...
        public void apply_chapter_support()
        {
            ...
            {
                List<int> unit_ids = new List<int>(Units.Keys);
                for (int i = 0; i < unit_ids.Count; i++)
                    for (int j = i + 1; j < unit_ids.Count; j++)
                    {
                        Units[unit_ids[i]].actor.chapter_support_gain(Units[unit_ids[j]].actor.id);
                        Units[unit_ids[j]].actor.chapter_support_gain(Units[unit_ids[i]].actor.id);
                    }
            }
        }
        ...
        {
            if (Constants.Gameplay.SIEGE_RELOADING && !Constants.Gameplay.SIEGE_MANUAL_RELOADING)
            {
                foreach (var pair in Siege_Engines)
                {
                    pair.Value.new_turn();
