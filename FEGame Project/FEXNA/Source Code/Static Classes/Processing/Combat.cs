...
namespace FEXNA
{
    ...
    class Combat
    {
        ...
        public static List<object> battle_setup(int id1, int id2, int distance, int mode)
        {
            ...
            switch (mode)
            {
                ...
                case 0:
                    ...
                    if (actor1.has_skill("ROLL") && weapon1.Id == 24)
                        actor1.attack_stacks++;
                    break;
                // Staff
                case 1:
            ...
            else
            {
                ...
                else
                {
                    ...
                    result.Add(Math.Max(0,
                        new BattlerStats(
                            battler_1.id, weapon1.Id, distance).dmg() -
                        (target as Destroyable_Object).def));
                    result.Add(0);
                    result.Add(null);
                }
            ...
            Game_Unit battler_1, Game_Unit battler_2, int distance, int actual_dmg, bool hit, bool crt, FEXNA_Library.Data_Weapon weapon)
        {
            ...
            result.hit = hit;
            result.crt = crt;
            int type = 1 + (int)weapon.Main_Type;
            ...
            if (result.hit)
            {
                if (result.crt)
                {
                    if(battler_2.actor.has_skill("DRAGONSKIN"))
                        actual_dmg = (int)Math.Floor(actual_dmg * 2f);
                    else if (battler_2.actor.has_skill("GUTS"))
                    {
                        actual_dmg = (int)Math.Floor(actual_dmg * (battler_1.actor.astraaaActivated ? 2 : Constants.Combat.CRIT_MULT));
                        if (battler_2.actor.has_skill("GUTS") && battler_2.actor.hp > 1 && actual_dmg >= battler_2.actor.hp)
                            actual_dmg = battler_2.actor.hp - 1;
                    }
                    else
                    {
                        actual_dmg = (int)Math.Floor(actual_dmg * (battler_1.actor.astraaaActivated ? 2 :  Constants.Combat.CRIT_MULT));
                ...
                if (battler_1.actor.has_skill("DESPSPEAR") && battler_1.actor.hp <= battler_1.actor.maxhp / 2)
                    actual_dmg *= 2;
                if (weapon.Cursed())
                ...
                else
                {
                    ...
                    result.kill = dmg >= hp;
                    state_change(weapon, ref result);
                    attack_buff_change(battler_1, battler_2);
                    if (result.hit && battler_2.actor.has_skill("VANDY2"))
                        battler_2.actor.vandy_stacks++;
        ...
        /// <summary>
        /// Gets an Attack_Result of one unit attacking a destroyable terrain object.
        /// </summary>
        /// <param name="battler_1">Attacking unit</param>
        ...
        /// <param name="weapon">Attacker's weapon</param>
        public static Attack_Result set_attack(
            Game_Unit battler_1, Destroyable_Object battler_2, int distance, int actual_dmg, bool hit, bool crt, FEXNA_Library.Data_Weapon weapon)
        {
            Attack_Result result = new Attack_Result { state_change = new List<KeyValuePair<int, bool>>() };
        ...
        public static void attack_buff_change(Game_Unit battler_1, Game_Unit battler_2)
        {
            //vandynerf
            if (battler_1.actor.has_skill("VANDYNERF"))
            {
                battler_2.set_stat_bonus(FEXNA_Library.Buffs.Pow, -1);
                battler_2.set_stat_bonus(FEXNA_Library.Buffs.Spd, -3);
            }
        }
        ...
        public static int exp(Game_Unit battler_1, Game_Unit battler_2, bool kill = false)
        {
            ...
            // Attack exp
            int exp_gain = Math.Max(1, (21 + (level_2 - level_1)) / 2);
            // Kill bonus
            if (kill)
            {
                exp_gain += Math.Max(1, (level_2 - level_1) + 20);
                if (battler_2.boss)
                    exp_gain += 30;
            }
            if (battler_1.actor.has_skill("CANTOFORCE"))
                exp_gain /= 2;
            if (battler_1.actor.has_skill("ANGLE"))
                exp_gain /= 8;
            if (battler_1.actor.has_skill("ANNOYING"))
                exp_gain = Math.Max(10, exp_gain);
            if (battler_1.aoe_exp_buff_skill())
                exp_gain = (int)(exp_gain * 1.5f);
            ...
            Data_Weapon weapon_1, Data_Weapon weapon_2, int distance)
        {
            ...
            // Check for override
            // Make Dominance always lose when there is one reaver
            //int? tri_override = battler_1.weapon_triangle_override(battler_2); //Yeti
            //if (tri_override != null)
            //    return (int)tri_override;
            ...
            WeaponType weapon_1_main = weapon_1.main_type();
            WeaponType weapon_1_scnd = weapon_1.scnd_type();
            WeaponType weapon_2_main = weapon_2 == null ? Global.weapon_types[0] : weapon_2.main_type();
            WeaponType weapon_2_scnd = weapon_2 == null ? Global.weapon_types[0] : weapon_2.scnd_type();
            ...
            {
                wpn_1_advs = new HashSet<int>(weapon_1_main.WtaReaverTypes.Union(weapon_1_scnd.WtaReaverTypes));
                wpn_2_advs = new HashSet<int>(weapon_2_main.WtaReaverTypes.Union(weapon_2_scnd.WtaReaverTypes));
            }
            else
            {
                wpn_1_advs = new HashSet<int>(weapon_1_main.WtaTypes.Union(weapon_1_scnd.WtaTypes));
                wpn_2_advs = new HashSet<int>(weapon_2_main.WtaTypes.Union(weapon_2_scnd.WtaTypes));
            }
            // Determines net weapon advantage
            foreach (int type in wpn_1_advs)
                if (type == (int)weapon_2.Main_Type || type == (int)weapon_2.Scnd_Type)
                    advantage1++;
            foreach (int type in wpn_2_advs)
                if (type == (int)weapon_1.Main_Type || type == (int)weapon_1.Scnd_Type)
                    advantage2++;
            // Modifies advantage based on range
            if (!(Global.game_system.In_Arena || Global.game_system.home_base) || Global.scene.is_test_battle)
            {
                if (weapon_1_main.WtaRanges.Contains(distance))
                    advantage1++;
                if (weapon_1_main.WtdRanges.Contains(distance))
                    advantage2++;

                if (weapon_1_scnd.WtaRanges.Contains(distance))
                    advantage1++;
                if (weapon_1_scnd.WtdRanges.Contains(distance))
                    advantage2++;

                if (weapon_2_main.WtaRanges.Contains(distance))
                    advantage2++;
                if (weapon_2_main.WtdRanges.Contains(distance))
                    advantage1++;

                if (weapon_2_scnd.WtaRanges.Contains(distance))
                    advantage2++;
                if (weapon_2_scnd.WtdRanges.Contains(distance))
                    advantage1++;
            }
            // Calculates result
        ...
        public static int weapon_triangle_mult(Game_Unit battler_1, Game_Unit battler_2, Data_Weapon weapon_1, Data_Weapon weapon_2, int distance)
        {
            ...
            if (weapon_1.Reaver() || weapon_2.Reaver())
                return 2;
            int skill_mult = battler_1.weapon_triangle_mult_skill(battler_2, weapon_1, weapon_2, distance);
            // Skills
            if (skill_mult > -1)
                return skill_mult;
            skill_mult = battler_2.weapon_triangle_mult_skill(battler_1, weapon_2, weapon_1, distance);
            if (skill_mult > -1)
                return skill_mult;
            return 1;
        }
        ...
        public static bool can_use_item(Game_Actor actor, int item_id, bool is_weapon, Game_Unit unit = null)
        {
            ...
            if (item.is_stat_booster())
            {
                ...
                if (item.Stat_Boost[(int)Boosts.WLvl] > 0 || item.Stat_Boost[(int)Boosts.WExp] > 0)
                    if (actor.equipped > 0)
                    {
                        WeaponType weapon_type = Global.data_weapons[actor.items[actor.equipped - 1].Id].main_type();
