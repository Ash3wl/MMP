    ...
    class Game_AI
    {
                ...
                foreach (int weapon_index in weapons)
                {
                    ...
                    if (weapon_index != Constants.Actor.NUM_ITEMS)
                    {
                                    ...
                                    if (retreat_loc.IsSomething)
                                    {
                                        ...
                                        attack_move_range = new HashSet<Vector2>(attack_move_range.Where(x =>
                                        {
                                            ...
                                            if (move_cost.IsNothing)
                                                ...
                                                return Pathfinding.get_distance(retreat_loc, attacker.id, attacker.mov - move_cost, false, x).IsSomething;
                                        }));
                                    }
                        ...
                        foreach (Vector2 loc in attack_move_range)
                        {
                            ...
                            {
                                Maybe<int> move_distance = Pathfinding.get_distance(loc, attacker.id, attacker.canto_mov, false, attacker.loc);
                    ...
                    else
                    {
                        ...
                        foreach (Siege_Engine siege in Global.game_map.siege_engines.Values)
                        {
                            ...
                            if (siege.is_ready && siege_move_range.Contains(siege.loc) && actor1.is_equippable(Global.data_weapons[siege.item.Id]))
                            {
                                Maybe<int> move_distance = Pathfinding.get_distance(siege.loc, attacker.id, attacker.canto_mov, false, attacker.loc);
                                if (move_distance.IsSomething)
                                {
            ...
            Vector2 loc, bool ever_counter, int distance, bool best, bool damage_test)
        {
            ...
            else
            {
                ...
                if (attacker.is_attackable_team(Constants.Team.PLAYER_TEAM))
                {
                    // If the target's death would cause the player to lose
                    if (Global.game_system.loss_on_death.Contains(actor2.id))
                        use[0] *= 1.5f;
                    else if (target.is_player_team && Global.game_system.ally_loss_on_death.Contains(actor2.id))
                        use[0] *= 1.5f;
                }
                // If on allied team
                else
            ...
            foreach (int id in targets)
            {
                ...
                foreach (int staff_index in staves)
                {
                    ...
                    foreach (Vector2 loc in attack_range)
                    {
                        ...
                        if (loc.X != Config.OFF_MAP.X)
                        {
                            Maybe<int> move_distance = Pathfinding.get_distance(loc, staff_user.id, staff_user.canto_mov, false, staff_user.loc);
                            if (move_distance.IsSomething)
                            {
        ...
        public static int[] get_untargeted_staff_target(Game_Unit staff_user, List<int> staves, bool can_move)
        {
            ...
            foreach (int staff_index in staves)
            {
                ...
                if (staff.Torch())
                {
                    ...
                    HashSet<Vector2> torch_range = Pathfinding.get_range_around(
                    ...
                    {
                        torch_range = Pathfinding.get_range_around(
        ...
        public static int[] get_steal_target(Game_Unit stealer, List<int> targets, HashSet<Vector2> move_range)
        {
            ...
            {
                Game_Unit target = Global.game_map.units[id];
                if (Pathfinding.hit_from_loc(target.loc, move_range, 1, 1).Count == 0)
                    continue;
        ...
        public static List<int> get_enemies_toward_target(Game_Unit unit, HashSet<Vector2> target_locs, List<int> targets, bool limited_access = false)
        {
            ...
            Pathfinding.ignore_units = true;
            List<Vector2> path = unit.actual_move_route(Pathfinding.get_route(target_distances[0].loc, -1, unit.id, unit.loc, true));
            Pathfinding.reset();
            // Remove targets that are further from the closest pillage location
        ...
        public static Maybe<Vector2>[] search_attack_through_walls(Game_Unit unit, HashSet<Vector2> enemy_tiles)
        {
            ...
            List<Vector2> total_move_range = new List<Vector2>(Pathfinding.get_range(unit.loc, -1, unit.id));
            if (enemy_tiles != null)
            {
                ...
                if (enemy_tiles != null)
                    ...
                    for (int i = 0; i < total_move_range.Count; i++)
                    {
                        ...
                        if (ranges.Contains(attack_distance) &&
                            (siege_ranges.Contains(attack_distance) || Pathfinding.clear_firing_line(total_move_range[i], target.loc)))
                        {
                            can_hit = true;
                            break;
                        }
        ...
        public static Maybe<Vector2>[] search_for_healing(Game_Unit unit)
        {
            ...
            foreach (Vector2 loc in Global.game_map.healing_terrain())
            {
                ...
                {
                    Maybe<int> check = Pathfinding.get_distance(loc, unit.id, -1, true);
        ...
        public static int[] search_for_rescue(Game_Unit unit, Vector2 actual_target_loc, bool heading_to_seize = false)
        {
            if (unit.boss || unit.is_rescue_blocked())
                return null;
            Maybe<int> distance_test = Pathfinding.get_distance(actual_target_loc, unit.id, -1, false);
            // Check for rescuing
            ...
            if (distance_test.IsSomething && distance_test >= unit.mov * 2)
            {
                ...
                //foreach (int unit_id in Global.game_map.teams[unit.team]) //Debug
                {
                            ...
                            (unit.mov >= target.mov + (unit.has_canto() ? 1 : 2)))
                    {
                        ...
                        distance_test = Pathfinding.get_distance(actual_target_loc, unit.id, -1, false, target.loc);
                        ...
                        if (distance_test.IsSomething && (distance_test - (unit.has_canto() ? (unit.mov - 1) / 2 : 0)) < distance)
                            ...
                            foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                            {
                                ...
                                {
                                    distance_test = Pathfinding.get_distance(target.loc + offset, unit.id, -1, false);
                    ...
                    foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                    {
                        ...
                        if (!Global.game_map.is_off_map(target.loc + offset) && !Global.game_map.is_blocked(target.loc + offset, unit.id))
                        {
                            ...
                            if (distance_test.IsSomething && distance_test <= unit.mov)
                            {
                                ...
                                distance_test = Pathfinding.get_distance(actual_target_loc, unit.id, -1, false, target.loc + offset);
        ...
        public static int[] search_for_rescue_drop(Game_Unit unit, Maybe<Vector2> move_loc, Maybe<Vector2> actual_target_loc)
        {
                ...
                (move_loc.IsSomething && Global.game_state.ai_enemy_attack_range.Contains(move_loc)))
            {
                ...
                HashSet<Vector2> blocked = new HashSet<Vector2>(move_range);
                foreach (Vector2 v in blocked)
                {
                    if (Global.game_map.is_blocked(v, unit.id, false))
                        move_range.Remove(v); // Obsoleted by the .Where<> above? //Debug
                }
                ...
                if (Global.game_system.Difficulty_Mode >= Difficulty_Modes.Hard)
                    foreach (Vector2 loc in move_range)
                    {
                        Maybe<int> check = Pathfinding.get_distance(actual_target_loc, unit.id, -1, true, loc);
                        if (check.IsSomething)
                        {
                            min_dist = -1;
                            foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                                if (!Global.game_state.ai_enemy_attack_range.Contains(loc + offset) && !Global.game_map.is_off_map(loc + offset) &&
                                    !Global.game_map.is_blocked(loc + offset, unit.id) && Pathfinding.passable(rescued_unit, loc + offset))
                                {
                                    check = Pathfinding.get_distance(actual_target_loc, unit.id, -1, true, loc + offset);
                ...
                if (target_locs.Count == 0)
                {
                    foreach (Vector2 loc in move_range)
                    {
                        ...
                        if (check.IsSomething)
                        {
                            min_dist = -1;
                            foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                                if (!Global.game_map.is_off_map(loc + offset) &&
                                    !Global.game_map.is_blocked(loc + offset, unit.id) && Pathfinding.passable(rescued_unit, loc + offset))
                                {
                                    check = Pathfinding.get_distance(actual_target_loc, unit.id, -1, true, loc + offset);
                                    if (check.IsSomething && (min_dist == -1 || check < min_dist))
                ...
                foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                {
                    if (!Global.game_map.is_off_map(target_loc + offset) &&
                        !Global.game_map.is_blocked(target_loc + offset, unit.id) && Pathfinding.passable(rescued_unit, target_loc + offset))
                    {
                        distance_test = Pathfinding.get_distance(actual_target_loc, unit.rescuing, -1, false, target_loc + offset);
                        if (distance_test.IsSomething)
        ...
        public static int[] search_for_savior(Game_Unit unit)
        {
            ...
            //foreach (int unit_id in Global.game_map.teams[unit.team]) //Debug
            {
                ...
                if (target.ai_terrain_healing && target.terrain_heals() &&
                    ...
                    unit.can_rescue(target) && Global.game_state.ai_enemy_attack_range.Contains(target.loc) && Pathfinding.passable(unit, target.loc))
                {
                        // Check if the unit can move to any tiles adjacent to the other unit
                        foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                        {
        ...
        public static int[] search_for_savior_drop(Game_Unit unit)
        {
                ...
                foreach (Vector2 loc in move_range)
                {
                    ...
                    foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                        ...
                        if ((Global.game_map.terrain_heals(loc + offset) || rescued_unit.actor.is_full_hp()) &&
                            ...
                            !Global.game_map.is_blocked(loc + offset, unit.id) && Pathfinding.passable(rescued_unit, loc + offset))
                        {
                            distance_test = Pathfinding.get_distance(loc, unit.id, -1, false);
            ...
            foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
            {
                if ((Global.game_map.terrain_heals(target_loc + offset) || rescued_unit.actor.is_full_hp()) && 
                    ...
                    !Global.game_map.is_blocked(target_loc + offset, unit.id) && Pathfinding.passable(rescued_unit, target_loc + offset))
                {
                    distance_test = Pathfinding.get_distance(target_loc, unit.id, -1, false);
                    if (distance_test.IsSomething)
                        target_locs.Add(new LocationDistance(target_loc + offset, distance_test));
                }
            }
        ...
        public static int[] search_for_talk(Game_Unit unit)
        {
            ...
            foreach (int target_id in potential_talkers)
            {
                ...
                foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                {
                    ...
                    if (!Global.game_map.is_off_map(target.loc + offset) && !Global.game_map.is_blocked(target.loc + offset, unit.id))
                    {
                        Maybe<int> distance_test = Pathfinding.get_distance(target.loc + offset, unit.id, -1, false);
                        if (distance_test.IsSomething && distance_test <= unit.mov)
                        {
            ...
            bool ignoreUnits = false)
        {
            foreach (int target_id in potentialTalkers)
            {
                ...
                foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                {
                    ...
                    {
                        Pathfinding.ignore_units = ignoreUnits;
                        Maybe<int> distance_test = Pathfinding.get_distance(target.loc + offset, unit.id, -1, false);
                        if (distance_test.IsSomething)
                        {
        ...
        public static int[] search_for_tile(Game_Unit unit)
        {
            ...
            {
                Maybe<int> check = Pathfinding.get_distance(Global.game_map.unit_seek_locs[unit.id], unit.id, -1, true);
                Pathfinding.reset();
                if (check.IsSomething)
                {
                    Maybe<Vector2> target_loc = path_to_target(unit, Global.game_map.unit_seek_locs[unit.id], offensive: false);
                    if (target_loc.IsSomething)
            ...
            {
                Maybe<int> check = Pathfinding.get_distance(Global.game_map.team_seek_locs[unit.team][unit.group], unit.id, -1, true);
                Pathfinding.reset();
                if (check.IsSomething)
                {
        ...
        {
            return path_to_target(unit, unit.loc, offensive: false, retreat: true, no_move_okay: no_move_okay, full_retreat: full_retreat);
        }
            ...
            bool ignore_doors = false)
        {
            ...
            foreach (int unit_id in target_team)
            {
                ...
                Maybe<int> check = Pathfinding.get_distance(target.loc, unit.id, -1, true, ignore_doors);
                if (check.IsSomething)
        ...
        {
            HashSet<Vector2> seize_points = Global.game_map.seize_points[unit.team];
            ...
            {
                Pathfinding.ignore_units = ignore_units;
                Maybe<int> check = Pathfinding.get_distance(seize_point, unit.id, -1, true);
                if (check.IsSomething)
            ...
            bool ignore_units = false, Vector2? base_loc = null)
        {
            ...
            {
                Pathfinding.ignore_units = ignore_units;
                Maybe<int> path = Pathfinding.get_distance(target_loc, unit.id, -1, true, loc); // through doors? and should also use the ignore_units bool //Yeti
                if (path.IsSomething)
            ...
            bool ignore_doors = false, bool ignore_blocking = false)
        {
            ...
            if (retreat)
            {
                // For other teams in the same team group as this unit
                var ally_seize_points = Constants.Team.TEAM_GROUPS
                    .First(x => x.Contains(unit.team))
                    .Where(x => x != unit.team)
                    // Get their seize points
                    .SelectMany(x => Global.game_map.seize_points[x]);
                // If any of those other teams have a seize point at the same tile as this unit's loc
            ...
            Pathfinding.reset();
            Maybe<int> check = Pathfinding.get_distance(target_loc, unit.id, -1, true, unit.loc, ignore_doors);
...
#endif
                Pathfinding.ignore_units = true;
                check = Pathfinding.get_distance(target_loc, unit.id, -1, true, unit.loc, ignore_doors);
            ...
            foreach (Vector2 loc in move_range)
            {
                Pathfinding.ignore_units = ignore_blocking || path_blocked_by_unit;
                check = Pathfinding.get_distance(target_loc, unit.id, -1, true, loc, ignore_doors);
                if (check.IsSomething)
                {
        ...
        {
            List<Vector2> route = unit.actual_move_route(loc, Pathfinding.get_route(target_loc, range, unit.id, loc, true, true));
            Pathfinding.reset();
            if (route.Count == 0)
                return null;
        ...
        internal static Maybe<Tuple<int, int>> healing_trade_target(Game_Unit unit)
        {
            ...
            if (unit.actor.has_critical_health() && !unit.can_heal_self())
                ...
                {
                    if (unit.same_team(Global.game_map.units[target_id]) &&
                        Global.game_map.units[target_id].can_heal_self(unit))
                    {
                        int item_index = Global.game_map.units[target_id].heal_self_item(unit);
                        if (trade_item_index == -1 || cost < Global.game_map.units[target_id].actor.items[item_index].to_item.Cost)
                        {
                            trade_target_id = target_id;
                            trade_item_index = item_index;
                            cost = Global.game_map.units[target_id].actor.items[item_index].to_item.Cost;
                        }
                    }
                }
        ...
        {
            HashSet<Vector2> result = Pathfinding.hit_from_loc(target.loc, move_range, 1, 1);
