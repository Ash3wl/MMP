...
namespace FEXNA
{
    ...
    public static partial class Global
    {
        ...
        internal static Save_Info current_save_info
        {
            get
            {
                return Save_Files_Info == null || !Save_Files_Info.ContainsKey(Current_Save_Id) ? null : Save_Files_Info[Current_Save_Id];
            }
        }
        ...
        public static void update_found(Tuple<Version, DateTime> result)
        {
            ...
            switch (scene)
            {
                ...
                case "Scene_Worldmap":
                    ...
                    if (Constants.WorldMap.SEPARATE_CHAPTERS_INTO_ARCS)
                        Scene = new SceneWorldmapArcs();
                    else
                        Scene = new Scene_Worldmap();
                    // Is this desired, compared to the default behavior of scrolling
                    // to the first unfinished chapter? //Yeti
                    if (scene != "Start_Game")
                    {
                        if (!string.IsNullOrEmpty(Game_System.chapter_id))
                            ((Scene_Worldmap)Scene).set_chapter(Game_System.chapter_id);
                        //if (Game_System.Chapter_Save_Progression_Keys.Length > 0) //Debug
                        //    ((Scene_Worldmap)Scene).set_chapter(Game_System.Chapter_Save_Progression_Keys[0]);
                        else if (!string.IsNullOrEmpty(chapter_id))
                            ((Scene_Worldmap)Scene).set_chapter(chapter_id);
                        else
                            ((Scene_Worldmap)Scene).refresh();
                    }
                    else
                        ((Scene_Worldmap)Scene).refresh();
                ...
                case "Scene_Worldmap_Unit_Editor":
                    Scene = new Scene_Worldmap_Unit_Editor();
                    ((Scene_Worldmap)Scene).refresh();
                    break;
                case "Scene_Save":
        ...
        public static Dictionary<string, string> chapter_text
        {
        ...
        public static Dictionary<string, string> global_text { get { return GlobalText; } }

        // Battle Text
        static Dictionary<string, string> Battle_Text;
        public static Dictionary<string, string> battle_text { get { return Battle_Text; } }

        // Death Quotes
        static Dictionary<string, string> Death_Quotes;
        public static Dictionary<string, string> death_quotes { get { return Death_Quotes; } }

        // Supports
        static Dictionary<string, string> Supports;
        public static Dictionary<string, string> supports { get { return Supports; } }

        // System Text
        static Dictionary<string, string> System_Text;
        public static Dictionary<string, string> system_text { get { return System_Text; } }

        // Actor Descriptions
        static Dictionary<string, string> Actor_Descriptions;
        public static Dictionary<string, string> actor_descriptions { get { return Actor_Descriptions; } }
        #endregion
        ...
        public static void write_game_system(System.IO.BinaryWriter writer)
        {
            Game_System.write(writer);
        }
        public static void write_events(System.IO.BinaryWriter writer)
        {
            Game_System.write_events(writer);
        }
        public static void read_game_system(System.IO.BinaryReader reader)
        {
            Game_System = new Game_System();
            Game_System.read(reader);
        }
        public static void read_events(System.IO.BinaryReader reader)
        {
