    ...
    class Ballista_Bolt : Map_Effect
    {
        ...
        public Ballista_Bolt(Vector2 src_loc, Vector2 dest_loc)
        {
            ...
            else
            {
                ...
                Angle = (float)Math.Atan((Math.Pow(FORCE, 2) + Math.Pow(Math.Pow(FORCE, 4) - (Math.Pow(GRAVITY * Total_Distance, 2)), 0.5f)) / (GRAVITY * Total_Distance));
            ...
            Scroll = Global.game_map.scroll_dist(dest_loc) / time;
            refresh();
        }
