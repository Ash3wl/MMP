...
namespace FEXNA
{
    class Convo_Backlog
    {
        ...
        //private int min_offset { get { return Config.WINDOW_HEIGHT - (Index * 16); } }
        private int min_offset { get { return Config.WINDOW_HEIGHT - (Config.CONVO_BACKLOG_LINES * 16); } }
        private int max_offset { get { return ((Index + 2) - Config.CONVO_BACKLOG_LINES) * 16; } }
        ...
        public Convo_Backlog()
        {
            ...
            {
                Text[i] = new FE_Text();
                Text[i].loc = new Vector2(24, (i - Config.CONVO_BACKLOG_LINES) * 16 + Config.WINDOW_HEIGHT + 8);
                Text[i].Font = "FE7_Convo";
                Text[i].stereoscopic = Config.CONVO_BACKLOG_DEPTH;
                Text[i].tint = Color.Transparent;
            }
        ...
        public void add_text(char c)
        {
            ...
            Text[Index].text_colors[Text[Index].text.Length - 1] = "FE7_Convo_" + color;*/
        }
        ...
        public void new_line()
        {
            ...
            if (!string.IsNullOrEmpty(LastColorSet))
                Text[Index].text_colors[0] = "FE7_Convo_" + LastColorSet;
        ...
        public void set_color(string color)
        {
            if (!started)
                throw new IndexOutOfRangeException("First convo backlog line not started");
            //Text[Index].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Convo_" + color);
            Text[Index].text_colors[Text[Index].text.Length] = "FE7_Convo_" + color;
            LastColorSet = color;
        }
        ...
        private void add_line(List<string> lines, List<string> colors)
        {
            ...
            for (int i = Math.Max(lines.Count - Config.CONVO_BACKLOG_LINES, 0); i < lines.Count; i++)
            {
                ...
                Text[Index].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Convo_" + colors[i]);
            }
        }
        ...
        public void fade_in(bool swipeIn = false)
        {
            ...
            {
                text.tint = swipeIn ? Color.Transparent : Color.White;
        ...
        public void fade_out(bool swipeOut = false)
        {
            ...
            foreach (var text in Text)
                text.tint = swipeOut ? Color.White : Color.Transparent;
        }
        ...
        public void update()
        {
            ...
            if (PanTimer > 0 && !Fading_In)
            {
                ...
                int alpha = Math.Min(255, PanTimer * 256 / Config.CONVO_BACKLOG_PAN_OUT_TIME);
            ...
            {
                FadeTimer--;
                Black_Fill.tint = new Color(0, 0, 0, ((Fading_In ? (Config.CONVO_BACKLOG_FADE_TIME - FadeTimer) : FadeTimer) * Config.CONVO_BACKLOG_BG_OPACITY) / Config.CONVO_BACKLOG_FADE_TIME);
            }
            // Pan in
            else if (PanTimer > 0)
            {
        ...
        public void draw(SpriteBatch sprite_batch, bool active)
        {
            ...
            sprite_batch.Begin(SpriteSortMode.Texture, BlendState.AlphaBlend);
            //if (FadeTimer == 0 && Fading_In) //Debug
            if ((FadeTimer == 0 && active) || !Fading_In)
