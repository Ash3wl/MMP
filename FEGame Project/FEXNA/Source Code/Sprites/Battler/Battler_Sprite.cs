...
namespace FEXNA
{
    class Battler_Sprite : Sprite
    {
        ...
        bool Using_Magic = false, Idle = false, Avoiding = false;
        int Shake = 0;
        ...
        {
            Battler = battler;
            Using_Magic = battler.magic_attack;
        ...
        public void anima_start(int distance)
        {
            ...
            if(Battler.actor.weapon_id == 105)
                spell_anim = anim_offset + 25;
        ...
        public void attack_spell(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim;
            anim = FE_Battler_Image_Wrapper.spell_attack_animation_value(Battler.actor.weapon_id, distance, hit);
            if (anim.Count > 0)
            {
            ...
            anim = FE_Battler_Image_Wrapper.spell_attack_animation_value_bg1(Battler.actor.weapon_id, distance, hit);
            if (anim.Count > 0)
            {
            ...
            anim = FE_Battler_Image_Wrapper.spell_attack_animation_value_bg2(Battler.actor.weapon_id, distance, hit);
            if (anim.Count > 0)
            {
            ...
            anim = FE_Battler_Image_Wrapper.spell_attack_animation_value_fg(Battler.actor.weapon_id, distance, hit);
            if (anim.Count > 0)
            {
        ...
        public void end_spell(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = FE_Battler_Image_Wrapper.spell_end_animation_value(Battler.actor.weapon_id, hit);
            if (anim.Count > 0)
            {
                Spell_Effect = new Battle_Animation(Name, null, anim, Reverse, false);
                Spell_Effect.loc = spell_loc;
        ...
        public void life_drain_spell_1(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = FE_Battler_Image_Wrapper.spell_lifedrain_animation_value(Battler.actor.weapon_id, distance);
            if (anim.Count > 0)
            {
                Spell_Effect = new Battle_Animation(Name, null, anim, Reverse, false);
                Spell_Effect.loc = spell_loc;
        ...
        public void life_drain_spell_2(bool hit, bool crit, int distance)
        {
            ...
            List<int> anim = FE_Battler_Image_Wrapper.spell_lifedrain_end_animation_value(Battler.actor.weapon_id);
            if (anim.Count > 0)
            {
                Spell_Effect = new Battle_Animation(Name, null, anim, Reverse, false);
                Spell_Effect.loc = spell_loc;
        ...
        protected void update_temp_anim()
        {
            ...
            if (Animation != null)
            {
                Dictionary<int, List<int>> added_effects = On_Hit.ADDED_EFFECTS(Animation.id);
                if (added_effects != null)
                    if (added_effects.ContainsKey(Animation.anim_timer))
                    {
                        List<int> anims = new List<int>(added_effects[Animation.anim_timer]);
