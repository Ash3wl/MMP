...
namespace FEXNA
{
    class SoloAnim_Page : Unit_Page
    {
        ...
        {
            Anim.text = FEXNA.Windows.Map.Window_Options.OPTIONS_DATA[
                (int)Options.Animation_Mode].Options[Mode].Value[0];
            Anim.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_"  +
                (Mode == (int)Animation_Modes.Map ? "Grey" : "Green"));
