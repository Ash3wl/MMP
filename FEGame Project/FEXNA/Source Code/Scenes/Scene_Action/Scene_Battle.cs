...
namespace FEXNA
{
    class Scene_Battle : Scene_Action
    {
        ...
        {
            if (Battler_1.continue_attacking()) return true;
            if (Battler_2.continue_attacking()) return true;
        ...
        {
            Global.Audio.play_bgm(Global.game_state.battle_theme());
        }

        protected override bool test_battle_theme()
        {
            string filename = Global.game_state.battle_theme();
            return Global.Audio.is_playing(filename);
        }
            ...
            switch (Segment)
            {
                case 0:
                    ...
                    while (!cont)
                    {
                        ...
                        bool magic_attack = battler_1.magic_attack;
                        if (!no_attack)
                        {
                            if (!Attack_Active)
                            {
                            ...
                            bool next_attack = attack(battler_1, battler_2, magic_attack);
                        ...
                        if (Timer >= 1)
                        {
                            ...
                            if (is_battler_dead(ids[1]))
                            {
                                if (Global.game_state.get_death_quote(
                                    battler_2_sprite.battler.id).Length > 0 && !is_test_battle)
                                {
                                    Dying_Unit = 2;
                            ...
                            {
                                if (Global.game_state.get_death_quote(
                                    battler_1_sprite.battler.id).Length > 0 && !is_test_battle)
                                {
                                    Dying_Unit = 1;
                                    insert_battle_action(0, Battle_Actions.Death_Quote);
                                }
                    ...
                    case Battle_Actions.Death_Quote:
                        ...
                        if (hit_over(magic_attack, battler_1_sprite))
                        {
                            ...
                            if (dying_unit.battler.is_player_team) //Multi
                            {
                                ...
                                if (Global.game_system.loss_on_death.Contains(dying_unit.battler.actor.id) ||
                                    Global.game_system.ally_loss_on_death.Contains(dying_unit.battler.actor.id))
                                {
        ...
        protected virtual void determine_hit(Battler_Sprite battler_1_sprite, Battler_Sprite battler_2_sprite)
        {
            ...
            Dmg_Dealt = Combat_Data.Data[Attack_Id].Key.Result.dmg > 0 || kill() || battler_1_sprite.battler.actor.astra_activated; //warkastra maybe
