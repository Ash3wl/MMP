...
namespace FEXNA
{
    ...
    partial class Scene_Action : Scene_Map
    {
        ...
        protected override void create_wlvl_popup(int weapon)
        {
            WLvl_Popup = new Weapon_Level_Popup(weapon);
            WLvl_Popup.loc = new Vector2(92, 64);
        }
        ...
        public override void draw(SpriteBatch sprite_batch, GraphicsDevice device, RenderTarget2D[] render_targets)
        {
            ...
            draw_battle(sprite_batch, device, render_targets[0], render_targets[1], render_targets[2], render_targets[3]);
        ...
        protected void draw_battle(SpriteBatch sprite_batch, GraphicsDevice device, RenderTarget2D map_render,
            RenderTarget2D final_render, RenderTarget2D temp_render, RenderTarget2D effect_render)
        {
            ...
            if (!level_up_layer_resort())
                draw_background_render(sprite_batch, device, final_render, effect_render, map_render);
            ...
            sprite_batch.Draw(temp_render, Vector2.Zero, new Color(160, 160, 160, 160));
            sprite_batch.End();
            ...
            {
                draw_masked_screen_darken(sprite_batch, battle_shader, final_render);
            }
            else
            {
                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
                sprite_batch.Draw(final_render, Vector2.Zero, Color.White);
                sprite_batch.End();
            }
            ...
            sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
            sprite_batch.Draw(temp_render, Vector2.Zero, Color.White);
            sprite_batch.End();
        ...
        private void draw_background_render(SpriteBatch sprite_batch, GraphicsDevice device,
            RenderTarget2D final_render, RenderTarget2D temp_render, RenderTarget2D map_render)
        {
            Effect battle_shader = Global.effect_shader();
            ...
            device.SetRenderTarget(map_render);
            ...
            // Draw map and backgrond to final render
            device.SetRenderTarget(final_render);
            sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend,
                SamplerState.PointClamp, null, null);
            sprite_batch.Draw(map_render, Vector2.Zero, Color.White);
            sprite_batch.End();
        }
