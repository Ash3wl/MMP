...
namespace FEXNA
{
    partial class Scene_Action
    {
        ...
        protected bool is_battle_bg_visible
        {
            get
            {
                return Constants.BattleScene.BATTLE_BG_ALWAYS_VISIBLE ||
                    Global.game_system.Battle_Mode == Animation_Modes.Full;
            }
        }
        ...
        protected virtual Texture2D platform(Game_Unit battler, int distance)
        {
            ...
            string terrain_name;
            if (Global.data_terrains[tag].Platform_Rename.Length > 0)
                terrain_name = Global.data_terrains[tag].Platform_Rename;
            else
                terrain_name = Global.data_terrains[tag].Name;

            if (Global.content_exists(string.Format(platform_format,
                    terrain_name + "_" + Global.game_map.tileset_battleback_suffix, range)))
                return battle_content.Load<Texture2D>(string.Format(platform_format,
                    terrain_name + "_" + Global.game_map.tileset_battleback_suffix, range));
            else if (Global.content_exists(string.Format(platform_format, terrain_name, range)))
                return battle_content.Load<Texture2D>(string.Format(platform_format, terrain_name, range));
            else
        ...
        protected virtual Texture2D background(Game_Unit battler)
        {
            ...
            if (Global.content_exists(string.Format(background_format,
                    terrain_name + "_" + Global.game_map.tileset_battleback_suffix)))
                return battle_content.Load<Texture2D>(string.Format(background_format,
                    terrain_name + "_" + Global.game_map.tileset_battleback_suffix));
            else if (Global.content_exists(string.Format(background_format, terrain_name)))
                return battle_content.Load<Texture2D>(string.Format(background_format, terrain_name));
            else
        ...
        protected virtual void update_phase_0()
        {
            ...
            while (!cont)
            {
                ...
                switch (Timer)
                {
                    ...
                    case 36:
                        battle_start_text();
                        if (!Music_Started && (Battler_1.boss || Battler_2.boss))// && (!test_battle_theme() || !Global.game_temp.boss_theme))
                            Global.Audio.bgm_fade(20);
                        Global.game_temp.boss_theme = false;
                        Timer++;
                        break;
                    case 41://wark yeti pls this waiting
                        if (!Music_Started)
                        {
                            if (Battler_1.actor.weapon_id == 224 || Battler_2.actor.weapon_id == 224)
                                Global.Audio.play_bgm("StrawberryCrisis");
                            else
                                play_battle_theme();
                            Music_Started = true;
                        }
        ...
        protected virtual void update_phase_3()
        {
            ...
            {
                if (Timer == 0)
                    if (Global.game_temp.boss_theme)//!Global.game_temp.boss_theme || Combat_Data.kill)
                    {
                        Global.Audio.bgm_fade(35);
                        Global.game_temp.boss_theme = false;
                    }
                if (Timer == 34)
                {
