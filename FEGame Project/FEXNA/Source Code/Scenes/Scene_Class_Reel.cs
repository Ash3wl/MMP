    ...
    class Scene_Class_Reel : Scene_Base
    {
        ...
        private void get_data_list()
        {
            ...
            for(int i = 0; i < Class_Reel_Data.ORDER.Length; i++)
                if (!already_added.Contains(Class_Reel_Data.ORDER[i]) && (string.IsNullOrEmpty(Class_Reel_Data.ORDER[i].Item1) ||
                            (Class_Reel_Data.ORDER[i].Item2 ?
                            Global.progress.completed_chapters[(int)Difficulty_Modes.Normal].Contains(Class_Reel_Data.ORDER[i].Item1) :
                            Global.progress.available_chapters[(int)Difficulty_Modes.Normal].Contains(Class_Reel_Data.ORDER[i].Item1))) &&
                        Class_Reel_Data.CH_DATA.ContainsKey(Class_Reel_Data.ORDER[i]))
                {
        ...
        protected void set_stats()
        {
            ...
            Message.text_speed = Message_Speeds.Normal;
            Message.silence();
            Message.block_skip();
                    ...
                    switch (Battler_Timer)
                    {
                        ...
                        case 0:
                            if (Battler.duration <= 1)
                            {
                                ...
                                if (Unit.magic_attack || Unit.actor.weapon_id==201)
                                {
                        ...
                        // Wait for hit
                        case 2:
                            if ((Unit.magic_attack|| Unit.actor.weapon_id==201) ? Battler.spell_ready : Battler.duration <= 1)
                            {
                                if (Unit.magic_attack || Unit.actor.weapon_id==201)
