...
namespace FEXNA
{
    enum Map_Menu_Options { Unit, Data, Options, Suspend, End }
    [Flags]
    ...
    partial class Scene_Map
    {
        ...
        Window_Command Unit_Command_Window, Map_Window;

        Window_Unit Unit_Window;
        Window_Data Data_Window;
        Window_Options Options_Window;

        Window_Command_Item Item_Window;
        Window_Item_Options Item_Options;
        Window_Command Discard_Confirm;
        Window_Command_Item Attack_Item_Window, Staff_Item_Window, Dance_Item_Window, Discard_Window, Repair_Item_Window;
        ...
        public bool confirming_something { get { return Map_Save_Confirm_Window != null || Leave_Base_Confirm_Window != null; } }
        #endregion
        ...
        protected virtual void open_unit_menu(Canto_Records canto)
        {
            ...
            if (canto_allows_normal_actions(Canto))
            {
                ...
                // Seize
                if (Global.game_map.seize_points[unit.team].Contains(unit.loc) && unit.can_seize)
                {
                    if (Global.game_system.chapter_id == "Pt1x")
                        commands.Add("Secure");
                    else
                        commands.Add("Seize");
        ...
        protected virtual void unit_menu_select(int option, Game_Unit unit)
        {
            ...
            switch (option)
            {
                ...
                case 2:
                    ...
                    {
                        Global.game_system.play_se(System_Sounds.Confirm);
                        Unit_Target_Window = new Window_Target_Rescue(unit.id, unit.is_rescuing ? 1 : 0, new Vector2(4, 0));
                        unit_command_window_active = false;
                        unit_command_window_visible = false;
                ...
                case 5:
                    Global.game_system.play_se(System_Sounds.Confirm);
                    //unit.Cantoforcing = false;
                    unit.end_cantoforcing();
                    unit.start_wait();
                    Global.game_temp.menuing = false;
        ...
        protected void update_attack_item_menu()
        {
            ...
            else
            {
                ...
                {
                    cancel_attack();
                    unit.equip(Attack_Item_Window.equipped);
                ...
                else if (Attack_Item_Window.is_selected())
                {
                    ...
                    unit.using_siege_engine = Attack_Item_Window.redirect() >=
                        Constants.Actor.NUM_ITEMS;
                    if (Global.game_options.combat_window == 1)
                        Unit_Target_Window = new Window_Target_Combat_Detail(Unit_Id,
                            unit.using_siege_engine ? Attack_Item_Window.redirect() : 0, new Vector2(4, 4));
                    else
                        Unit_Target_Window = new Window_Target_Combat(Unit_Id,
                            unit.using_siege_engine ? Attack_Item_Window.redirect() : 0, new Vector2(4, 4));
        ...
        protected void confirm_combat()
        {
            ...
            {
                Global.game_state.call_aoe(Unit_Id, unit.enemies_in_old_swoop_range(unit.facing)[0]);
                sort_aoe_targets();
        ...
        protected void update_staff_item_menu()
        {
            ...
            else
            {
                ...
                {
                    cancel_staff();
                    unit.equip(Staff_Item_Window.equipped);
                    unit.actor.staff_fix();
                    List<int> item_indices = unit.weapon_indices(Useable_Staves);
                    Global.game_temp.temp_staff_range = unit.get_weapon_range(item_indices, new HashSet<Vector2> { unit.loc });
                    //Global.game_temp.temp_staff_range = Global.game_map.get_unit_range(new List<Vector2> { unit.loc },
        ...
        protected void confirm_staff()
        {
            ...
            Game_Unit unit = Global.game_map.units[Unit_Id];
            unit.moved();
            Global.game_state.call_staff(Unit_Id, Unit_Target_Window.target,
                Unit_Target_Window.manual_targeting ? Global.player.loc : Global.game_map.attackable_map_object(Unit_Target_Window.target).loc);
            // Adds to list of attacked units for this turn
            if (Global.game_map.units.ContainsKey(Global.game_system.Battler_2_Id) &&
                    unit.is_attackable_team(Global.game_map.units[Global.game_system.Battler_2_Id]))
            {
        ...
        protected void update_repair_item_menu()
        {
            ...
            else
            {
                ...
                {
                    unit.equip(Repair_Item_Window.equipped);
                    unit.actor.staff_fix();
                    Global.game_map.range_start_timer = 0;
                    Global.game_system.play_se(System_Sounds.Cancel);
                    Item_Options.active = true;
            ...
            else
            {
                ...
                else if (Trade_Window.is_canceled() || cancel_button_triggered)
                {
                    ...
                    else if (Traded)
                    {
                        ...
                        //post-trade cantoforce stuff goes here
        ...
        {
            Unit_Command_Window = new Window_Command(new Vector2(8 + (Global.player.is_on_left() ? (Config.WINDOW_WIDTH - (width + 16)) : 0), 24), width, commands);
        ...
        {
            List<string> commands = new List<string> { "Unit", "Data", "Options", "Suspend", "End" };
            new_map_window(commands, 56);
        }

        protected virtual void update_map_menu()
        {
#if DEBUG
            if (DebugMenuOpen)
            {
            ...
            else if (map_command_is_selected)
            {
                switch ((Map_Menu_Options)map_window_selected_index.ValueOrDefault)
                {
        ...
        // Unit
        protected void new_unit_window()
        {
            Unit_Window = unit_window();
        }

        internal virtual Window_Unit unit_window()
        {
            return new Window_Unit();
        }

        protected void update_unit_menu()
        {
            if (Unit_Window.closed)
            {
                if (Global.game_system.preparations && !Global.game_system.home_base)
                {
                ...
                {
                    if (Unit_Window.unit_selected)
                        Global.player.force_loc(Global.game_map.units[Unit_Window.unit_index].loc);
                    if (Options_Window == null)
                        Global.game_map.highlight_test();
        ...
        // Data
        protected void update_data_menu()
        {
            if (Data_Window.closed)
            {
                Global.game_temp.menuing = false;
                Global.game_map.highlight_test();
                Data_Window = null;
        ...
        // Options
        protected void new_options_window()
        {
            Options_Window = new Window_Options();
        }

        protected void update_options_menu()
        {
            ...
            else if (Options_Window.soloanim_call)
            {
                Options_Window.soloanim_call = false;
                Unit_Window = new Window_SoloAnim();
            }
        }
        ...
        protected void new_map_window(List<string> commands, int width)
        {
            Map_Window = new Window_Command(new Vector2(8 + (Global.player.is_on_left() ? (Config.WINDOW_WIDTH - (width + 16)) : 0), 24), width, commands);
            Map_Window.stereoscopic = Config.MAPCOMMAND_WINDOW_DEPTH;
            Map_Window.help_stereoscopic = Config.MAPCOMMAND_HELP_DEPTH;
            if (Global.game_temp.end_turn_highlit)
            {
                Map_Window.immediate_index = (int)Map_Menu_Options.End;
...
#if DEBUG
        protected void new_map_debug_window(List<string> commands, int width)
        {
            Map_Window = new Window_Command_Scrollbar(
                new Vector2(8 + (Global.player.is_on_left() ? (Config.WINDOW_WIDTH - (width + 16)) : 0), 24),
                width, 8, commands);
            Map_Window.stereoscopic = Config.MAPCOMMAND_WINDOW_DEPTH;
            Map_Window.help_stereoscopic = Config.MAPCOMMAND_HELP_DEPTH;
        ...
        protected int map_window_color { set { Map_Window.color_override = value; } }

        protected bool is_map_window_open { get { return Map_Window != null; } }

        protected void update_map_window()
        {
            Map_Window.update();
        ...
        protected int map_window_index { get { return Map_Window.index; } }
        protected Maybe<int> map_window_selected_index { get { return Map_Window.selected_index(); } }

        protected bool map_command_is_selected { get { return map_window_selected_index.IsSomething; } }

        protected bool map_command_is_canceled
        {
            get
            {
                return Map_Window.is_canceled() ||
                    cancel_button_triggered ||
                    Global.Input.gesture_triggered(TouchGestures.Tap);
            }
        }

        protected Vector2 map_window_loc { get { return Map_Window.loc; } }

        protected void set_map_window_text_color(int index, string color)
        {
            Map_Window.set_text_color(index, color);
        }

        protected bool map_window_active
        {
            get { return Map_Window.active; }
            set { Map_Window.active = value; }
        }

#if DEBUG
        enum Debug_Menu_Options { RefreshUnit, DeleteUnit, SkipChapter, MaxSupport, LevelUp, ToggleFog, InfiniteMove }
        ...
        internal virtual void open_debug_menu()
        {
                ...
                Minimap == null)
            {
                ...
                List<string> commands = new List<string> {
                    "Refresh", "Delete Unit", "Skip Chapter", "Support++", "Level Up",
                    "Toggle Fog", "Toggle Inf Mov" };
                new_map_debug_window(commands, 96);

                Game_Unit unit = Global.game_map.get_unit(Global.player.loc);
                if (unit == null)
                {
                    Map_Window.set_text_color((int)Debug_Menu_Options.RefreshUnit, "Grey");
                    Map_Window.set_text_color((int)Debug_Menu_Options.DeleteUnit, "Grey");
                    Map_Window.set_text_color((int)Debug_Menu_Options.MaxSupport, "Grey");
                    Map_Window.set_text_color((int)Debug_Menu_Options.LevelUp, "Grey");
                }
        ...
        private void update_debug_menu()
        {
            if (map_command_is_canceled)
            {
                Global.game_system.play_se(System_Sounds.Cancel);
            ...
            {
                Game_Unit unit = Global.game_map.get_unit(Global.player.loc);
                switch ((Debug_Menu_Options)map_window_selected_index.ValueOrDefault)
                {
                    case Debug_Menu_Options.RefreshUnit:
                        if (unit == null)
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        else
                        {
                        ...
                        break;
                    case Debug_Menu_Options.DeleteUnit:
                        if (unit == null)
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        else
                        {
                        ...
                        break;
                    case Debug_Menu_Options.SkipChapter:
                        Global.Audio.play_se("System Sounds", "Confirm");
                        Global.game_temp.menuing = false;
                        close_map_menu();
                        DebugMenuOpen = false;
                        ...
                        break;
                    case Debug_Menu_Options.MaxSupport:
                        if (unit == null)
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        else
                        {
                        ...
                        break;
                    case Debug_Menu_Options.LevelUp:
                        if (unit == null)
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        else
                        {
                        ...
                        break;
                    case Debug_Menu_Options.ToggleFog:
                        ...
                        Global.game_map.highlight_test();
                        break;
                    case Debug_Menu_Options.InfiniteMove:
        ...
        protected void update_status_menu()
        {
            if (Status_Window.closed)
            {
                ...
                else if (Prep_PickUnits_Window != null)
                    Prep_PickUnits_Window.actor_id = Status_Window.current_actor;
                else if (Prep_Items_Window != null)
                    Prep_Items_Window.actor_id = Status_Window.current_actor;
                else if (is_unit_command_window_open)
                {
                    unit_command_window_visible = true;
                    unit_command_window_active = true;
                }
                ...
                if (((Window_Command_Item_Discard)Discard_Window).confirm_ready)
                {
                    ...
                    else if ((Discard_Window as Window_Command_Item_Discard).confirm_is_selected())
                    {
                        ...
                        switch (((Window_Command_Item_Discard)Discard_Window).confirm_index)
                        {
                            // Yes
                            case 0:
                                unit.equip(Discard_Window.equipped);
                                FEXNA_Library.Item_Data discarded_item = unit.actor.items[Discard_Window.redirect()];
                                if (Global.battalion.convoy_ready_for_sending || (discarded_item.Id == 6) && discarded_item.is_item)
                                {
        ...
        protected void open_shop_menu()
        {
            ...
            {
                Shop_Window = new Window_Shop(Global.game_system.Shopper_Id, Global.game_battalions.active_convoy_shop);
            }
            else
            {
        ...
        {
            if (Global.game_system.preparations && Prep_Items_Window != null)
                Prep_Items_Window.refresh();
        }
        ...
        {
            draw_preparations(sprite_batch);
            if (is_map_window_open || is_unit_command_window_open)
            {
                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
            ...
            if (is_map_window_open)
                Map_Window.draw(sprite_batch);
            if (is_unit_command_window_open)
                Unit_Command_Window.draw(sprite_batch);

            if (Data_Window != null) Data_Window.draw(sprite_batch);
            if (Options_Window != null) Options_Window.draw(sprite_batch);
