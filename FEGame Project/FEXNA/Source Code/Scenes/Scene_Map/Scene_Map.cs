...
namespace FEXNA
{
    ...
    internal partial class Scene_Map : Scene_Level_Up
    {
        ...
        {
            if (item_data == null)
                set_popup("Failed to steal!", 70, 84);
            else
        ...
        public void re_add_map_sprites()
        {
            ...
            foreach (KeyValuePair<int, Game_Unit> unit in Global.game_map.units)
                unit.Value.init_sprites();
            foreach (KeyValuePair<int, Siege_Engine> siege in Global.game_map.siege_engines)
                siege.Value.init_sprites();
        }
        ...
        protected void draw_ranges(SpriteBatch sprite_batch)
        {
            if (Global.game_map.move_range_visible)
            {
                ...
                if (Global.game_map.is_enemy_range_visible() &&
                        (Global.game_map.all_enemy_displayed_staff.Count > 0 || Global.game_map.enemy_displayed_staff.Count > 0 ||
                ...
                if (Global.game_map.is_enemy_range_visible() && Global.game_system.Selected_Unit_Id == -1)
        ...
        protected void draw_auras(SpriteBatch sprite_batch)
        {
            ...
            foreach (Color tint in Game_Unit.AURA_COLOR_ORDER)
            {
                ...
                foreach (int unit_id in aura_units)
                    if (Global.game_map.units[unit_id].aura_color() == tint)
                    {
                        // Gets the area around the unit for the aura
                        HashSet<Vector2> locs = Pathfinding.get_range_around(
                        ...
                        {
                            if (Global.game_map.fow && Global.game_map.units[unit_id].is_attackable_team(Constants.Team.PLAYER_TEAM) &&
        ...
        protected virtual void draw_units(SpriteBatch sprite_batch, bool rotated = false, bool roof = false, HashSet<Vector2> roof_tiles = null)
        {
            ...
            draw_siege(sprite_batch, roof, roof_tiles);
        ...
        protected void draw_siege(SpriteBatch sprite_batch, bool roof = false, HashSet<Vector2> roof_tiles = null)
        {
            // Begin siege sprite batch
            ...
            if (sprite_batch.GraphicsDevice.ScissorRectangle.Width > 0 && sprite_batch.GraphicsDevice.ScissorRectangle.Width > 0)
            {
                ...
                foreach (int id in siege_keys)
                {
                    ...
                    if ((!Global.game_map.fow || Global.game_map.fow_visibility[Constants.Team.PLAYER_TEAM].Contains(Global.game_map.siege_engines[id].loc)) && (
                            unit != null && !unit.sprite_moving && !unit.battling && unit.is_on_siege()))
                        continue;
                ...
                if (spriteBatch.GraphicsDevice.ScissorRectangle.Width > 0 && spriteBatch.GraphicsDevice.ScissorRectangle.Width > 0)
                {
                    ...
                    if (Global.game_system.Selected_Unit_Id != -1)
                        foreach (int id in units)
                        {
                            ...
                            //else if (selected_unit.actor.get_support_level(unit.actor.id) > 0)
                            if (this.combat_target_window_up &&
                                ...
                                selected_unit.actor.get_support_level(unit.actor.id) > 0))
                            {
                                ...
                                if (Global.game_map.unit_distance(selected_unit.id, unit.id) > Constants.Support.SUPPORT_RANGE)
                                    tint = new Color(192, 192, 192, 255);
        ...
        protected virtual void draw_active_units(SpriteBatch sprite_batch)
        {
            ...
            foreach (int id in DeferredUnitIds)
            {
                ...
                if (sprite_batch.GraphicsDevice.ScissorRectangle.Width > 0 && sprite_batch.GraphicsDevice.ScissorRectangle.Width > 0)
                {
                    ...
                    if (Global.game_map.range_enemies.Contains(id) && Global.game_map.is_enemy_range_visible() && Global.game_map.move_range_visible)
                    {
                        enemy_range_unit_tint(id);
                    }
