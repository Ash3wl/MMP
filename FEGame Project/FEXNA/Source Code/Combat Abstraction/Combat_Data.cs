...
namespace FEXNA
{
    internal class Combat_Data
    {
        ...
        protected virtual void setup()
        {
            ...
            battler_1.actor.store_states();
            if (battler_2 != null)
                battler_2.actor.store_states();
            ...
            end_battle(battler_1, battler_2);
            set_exp(battler_1, battler_2);
            battler_1.actor.restore_states();
            if (battler_2 != null)
                battler_2.actor.restore_states();
        }
        ...
        private void cap_exp_gain(Game_Unit battler, Game_Unit target, ref int expGain)
        {
            if (expGain != 0)
            {
                ...
                if (expGain > 0 && exp_gain_cap(target))
                    expGain = Math.Min(expGain,
                        Constants.Actor.EXP_PER_ENEMY - target.exp_given);
            }
        }
        ...
        protected virtual bool battler_can_gain_exp(Game_Unit battler, Game_Unit target = null)
        {
            ...
            if (!(battler.is_ally ||
                    (Constants.Actor.CITIZENS_GAIN_EXP &&
                    !battler.actor.is_generic_actor &&
                    !battler.is_attackable_team(Constants.Team.PLAYER_TEAM))))
                return false;
        ...
        protected virtual void set_variables(Game_Unit battler_1, Destroyable_Object battler_2)
        {
            Hp1 = battler_1.actor.hp;
            MaxHp1 = battler_1.actor.maxhp;
            Team1 = battler_1.team;
            ...
            if (battler_2 != null)
            {
                ...
                Team2 = Constants.Map.DESTROYABLE_OBJECT_TEAM;
        ...
        protected virtual void process_attacks(Game_Unit battler_1, Combat_Map_Object battler_2)
        {
            bool ambush = battler_2 != null && battler_2.is_unit() &&
                Global.game_map.units[(int)Battler_2_Id].counters_first(Global.game_map.units[Battler_1_Id]);
            ...
            while (!cont)
            {
                ...
                if (attacker_index > 0)
                {
                    if (battler_2 == null)
                        ...
                        add_attack(battler_1, battler_2 as Destroyable_Object, attacker_index);
                }
        ...
        protected void add_attack(Game_Unit battler_1, Game_Unit battler_2, int attacker)
        {
            ...
            if (attacker == 1)
                ...
                Attacked_2 = true;
            Data[Data.Count - 1].Key.set_attack(battler_1, battler_2, Distance, Data[Data.Count - 1].Value);
            // Add Attacks
        ...
        protected void add_attack(Game_Unit battler_1, Destroyable_Object battler_2, int attacker)
        {
            add_data(battler_1, battler_2);
            Data[Data.Count - 1].Key.Attacker = attacker;
            // Checks if this is the first attack of this unit
            ...
            if (attacker == 1)
                ...
                Attacked_2 = true;
            ((Terrain_Round_Data)Data[Data.Count - 1].Key).set_attack(battler_1, battler_2, Distance, Data[Data.Count - 1].Value);
            // Add Attacks
            if (battler_1.actor.added_attacks.Count > 0)
                foreach (int attack in battler_1.actor.added_attacks)
                    Add_Attack.Add(attack == 1 ? 1 : 2);
            ...
            Weapon_1_Uses += battler_1.weapon_use_count(Data[Data.Count - 1].Key.Result.hit);
        }
        protected void add_solo_attack(Game_Unit battler_1, int attacker)
        {
            ...
            if (attacker == 1)
                ...
                Attacked_2 = true;
            Data[Data.Count - 1].Key.set_attack(battler_1, null, Distance, Data[Data.Count - 1].Value);
            // Add Attacks
            if (battler_1.actor.added_attacks.Count > 0)
                foreach (int attack in battler_1.actor.added_attacks)
                    Add_Attack.Add(attack == 1 ? 1 : 2);
            ...
            Data[Data.Count - 1].Key.cause_damage(Battler_1_Id);
        }
            ...
            KeyValuePair<Combat_Round_Data, List<Combat_Action_Data>> data)
        {
            ...
            {
                Exp_Gain1 = Math.Max(0, Exp_Gain1);
                Weapon_1_Uses += battler_1.weapon_use_count(data.Key.Result.hit);
            ...
            else
            {
                ...
                Weapon_2_Uses += battler_2.weapon_use_count(data.Key.Result.hit);
        ...
        protected virtual void add_data(Game_Unit battler_1, Destroyable_Object battler_2)
        {
            Data.Add(new KeyValuePair<Combat_Round_Data, List<Combat_Action_Data>>(
                new Terrain_Round_Data(battler_1, battler_2, Distance), new List<Combat_Action_Data>()));
        }

        protected virtual void end_battle(Game_Unit battler_1, Combat_Map_Object battler_2)
        {
            Data[Data.Count - 1].Key.end_battle(battler_1, battler_2, Distance, Data[Data.Count - 1].Value);
        }
        #endregion
        ...
        protected virtual bool attackable(Game_Unit attacker, Combat_Map_Object target)
        {
            ...
            if (target.is_unit())
            {
                ...
                //if (target_unit.actor.hp <= 0) return null; //Debug
            }
            //else //Debug
            //    if (((Destroyable_Object)target).hp <= 0) return null;
        ...
        public virtual void apply_combat(bool immediate_level = false, bool skip_exp = false)
        {
            ...
            for(int i = 0; i < Data.Count; i++)
            {
                ...
                if (Battler_2_Id == null)
                    data.cause_damage(Battler_1_Id);
                else
                    data.cause_damage(Battler_1_Id, (int)Battler_2_Id);
            }
            Global.game_state.add_combat_metric(this is Staff_Data ? Metrics.CombatTypes.Staff : Metrics.CombatTypes.Battle,
            ...
            if (battler_1.actor.has_skill("INOX"))
                battler_1.actor.attack_stacks = (int)Math.Ceiling(battler_1.actor.attack_stacks / 2f);
            ...
            if (!skip_exp)
            {
                if (Wexp1 > 0)
                {
                    WeaponType type = Global.data_weapons[Weapon_1_Id].main_type();
                    battler_1.actor.wexp_gain(type, Wexp1);
                }
                if (Wexp2 > 0)
                {
                    WeaponType type = Global.data_weapons[(int)Weapon_2_Id].main_type();
        ...
        protected void use_weapons(Game_Unit battler_1, Game_Unit battler_2)
        {
            ...
            if (battler_1.actor.weapon != null &&
                    !battler_1.actor.is_equippable(battler_1.actor.weapon))
                battler_1.actor.setup_items(false);
            if (battler_2 != null)
            {
                if (battler_2.actor.weapon != null &&
                        !battler_2.actor.is_equippable(battler_2.actor.weapon))
