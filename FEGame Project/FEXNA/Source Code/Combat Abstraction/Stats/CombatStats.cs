    ...
    class CombatStats : BattlerStats
    {
        ...
        private int dmg_target(Game_Unit target)
        {
            ...
            else
            {
                ...
                target_def = ((weapon.Ignores_Def() && !target.actor.has_skill("SALAMI")) ? target_def / 2 : target_def);
                if (weapon.Halves_HP())
                {
                ...
                else
                {
                    ...
                    /*if (weapon.Ignores_Def() && !target.actor.has_skill("SALAMI"))
                        weapon_dmg += target_def;
                    else if (weapon.Ignores_Def())
                        weapon_dmg += target_def / 2;*/
                }
                // Weapon triangle
                skill_dmg = 0;
        ...
        private int hit_target(Game_Unit target)
        {
            ...
            else
            {
                ...
                attacker.hit_skill(
                    ...
                    weapon, target, magic_attack, Distance);
                support_hit = support_bonus(Combat_Stat_Labels.Hit);
                s_bonus = attacker.actor.s_rank_bonus(weapon.main_type());
                // Weapon triangle
                WeaponTriangle tri = Combat.weapon_triangle(attacker, target, weapon, weapon2, Distance);
                if (tri != WeaponTriangle.Nothing)
            ...
            int result = attacker.hit_target_skill(target, weapon, Distance, Math.Max(total_hit, 0) / (boss_staff ? 2 : 1));
            return result;
        }
        #endregion
        ...
        private int crt_target(Game_Unit target)
        {
            ...
            if (Constants.Combat.HIT_OVERFLOW || attacker.actor.has_skill("BACKSWORD"))
            {
