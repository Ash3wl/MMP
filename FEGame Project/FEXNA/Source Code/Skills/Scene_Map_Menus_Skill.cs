...
namespace FEXNA
{
    ...
    partial class Scene_Map
    {
        ...
        protected void skill_commands(ref List<string> commands, Game_Unit unit)
        {
            ...
            {
                int index;
                /*int index = Math.Min(commands.IndexOf("Status"), commands.IndexOf("Wait"));
                commands.Insert(index + 0, "Dupe");
                add_skill_index(index - 1, Skill_Menu_Ids.Dupe);*/
                if (commands.Contains("Staff"))
                {
                    index = commands.IndexOf("Staff");
                    commands.Insert(index + 1, "Dupe");
                    add_skill_index(index + 0, Skill_Menu_Ids.Dupe);
                }
                else if (commands.Contains("Attack"))
                {
                    index = commands.IndexOf("Attack");
                    commands.Insert(index + 1, "Dupe");
                    add_skill_index(index + 0, Skill_Menu_Ids.Dupe);
                }
                else
            ...
            if (unit.actor.id == 1 && !unit.is_rescuing)
            {
                int index = Math.Min(commands.IndexOf("Status"), commands.IndexOf("Wait"));
                commands.Insert(index + 0, "Hiatus");
                add_skill_index(index - 1, Skill_Menu_Ids.Hiatus);
            }
            if (unit.actor.has_skill("WOLFRAM") && unit.is_mastery_ready("WOLFRAM"))
            {
                int index = Math.Min(commands.IndexOf("Status"), commands.IndexOf("Wait"));
                commands.Insert(index + 0, "Dekagle");
                add_skill_index(index - 1, Skill_Menu_Ids.Angel);
            }
            ...
            if (canto_allows_normal_actions(Canto))
            {
                ...
                if (unit.actor.has_skill("FRANZ"))
                {
                    int index = Math.Min(commands.IndexOf("Status"), commands.IndexOf("Wait"));
                    if (unit.actor.class_id == 89)
                    {
                        commands.Insert(index + 0, "Mount");
                        add_skill_index(index - 1, Skill_Menu_Ids.Mount);
                    }
                    else
                    {
                        commands.Insert(index + 0, "Dismount");
                        add_skill_index(index - 1, Skill_Menu_Ids.Dismount);
                    }
                ...
                if (commands.Contains("Rescue") && unit.has_cover())
                {
                    ...
                    foreach (int id in ally_range)
                        if (unit.can_rescue(Global.game_map.units[id]))
                            if (Pathfinding.passable(unit, Global.game_map.units[id].loc))
                            {
                                can_rescue = true;
                                break;
                            }
                ...
                {
                    if (unit.actor.has_skill(Game_Unit.MASTERIES[i]) && unit.is_mastery_ready(Game_Unit.MASTERIES[i]) && unit.actor.id != 10)
                    {
        ...
        protected bool unit_menu_select_skill(int option, Game_Unit unit)
        {
            ...
            if (option >= BASE_MASTERY_MENU_ID && option < BASE_MASTERY_MENU_ID + Game_Unit.MASTERIES.Count)
            {
                ...
                switch (option)
                {
                    case BASE_SKILL_MENU_ID + (int)Skill_Menu_Ids.Dupe:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        //close_unit_menu(true);
                        //Global.game_system.call_event("Duplicate", true);
                        unit.actor.add_state(21);
                        //unit.wait(true);
                        //close_unit_menu(true);
                        // Value[0] = id
                        //?Value[1] = ready
                        ...
                        break;
                    // Skills: Hiatus
                    case BASE_SKILL_MENU_ID + (int)Skill_Menu_Ids.Hiatus:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        if (unit.actor.states.Contains(31))
                            unit.actor.remove_state(31);
                        else
                            unit.actor.add_state(31);
                        unit.start_wait();
                        Global.game_temp.menuing = false;
                        close_unit_menu(true);
                        break;
                    case BASE_SKILL_MENU_ID + (int)Skill_Menu_Ids.Angel:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        //close_unit_menu(true);
                        //Global.game_system.call_event("Duplicate", true);
                        unit.actor.add_state(33);
                        unit.uncharge_masteries("WOLFRAM");
                        //unit.wait(true);
                        //close_unit_menu(true);
                        // Value[0] = id
                        //?Value[1] = ready
                    ...
                    case BASE_SKILL_MENU_ID + (int)Skill_Menu_Ids.Mount:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        //close_unit_menu(true);
                        //Global.game_system.call_event("Duplicate", true);
                        //unit.actor.add_state(34);
                        Global.game_system.SWITCHES[41] = true;
                        //unit.wait(true);
                        //close_unit_menu(true);
                        // Value[0] = id
                        //?Value[1] = ready
                        ...
                        //suspend();
                        break;
                    case BASE_SKILL_MENU_ID + (int)Skill_Menu_Ids.Dismount:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        //close_unit_menu(true);
                        //Global.game_system.call_event("Duplicate", true);
                        //unit.actor.add_state(34);
                        Global.game_system.SWITCHES[41] = false;
                        //unit.wait(true);
                        //close_unit_menu(true);
                        // Value[0] = id
                        //?Value[1] = ready
