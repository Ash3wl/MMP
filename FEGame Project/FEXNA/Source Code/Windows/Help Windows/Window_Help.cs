...
namespace FEXNA
{
    class Window_Help : Sprite
    {
        ...
        public override void update()
        {
            ...
            update_size();
            if (Resize.Count <= 4 && Locs.Count <= 4)
            {
                if (Help_Text != null)
                    if (Text_Timer <= 0)
                    {
        ...
        {
            return Global.game_options.text_speed == (int)Message_Speeds.Max;
        }

        protected virtual int text_speed()
        {
            return Global.game_options.text_speed == (int)Message_Speeds.Slow ? 2 : 1;
        }

        protected virtual int characters_at_once()
        {
            return (Global.game_options.text_speed == (int)Message_Speeds.Fast ? 2 : 1);
        }
        ...
        public virtual void set_text(string text, Vector2 offset = new Vector2())
        {
            ...
            Help_Text.Font = "FE7_Convo";
            Help_Text.texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Convo_Black");
            Help_Text.text = "";
            ...
            foreach (string str in text_ary)
                width = Math.Max(Font_Data.text_width(str, "FE7_Convo"), width);
