...
namespace FEXNA
{
    enum PrepItemsInputResults { None, OpenTrade, Status, Supply, Shop, Closing }
    ...
    internal class Window_Prep_Items : Windows.Map.Map_Window_Base
    {
        ...
        public bool ready
        {
            get
            {
                ...
                return !Closing && Black_Screen_Timer <= 0;
            }
        }
        ...
        protected virtual void returning_to_menu(bool returning_to_item_use)
        {
            if (returning_to_item_use)
            {
                ...
                Command_Window.immediate_index = 2;
            }
        }
        ...
        {
            List<string> strs = new List<string> { "Trade", "List", "Use", "Give All", "Convoy", "Restock" };
        ...
        protected virtual void refresh_convoy_use_color()
        {
            if (actor.can_use_convoy_item())
                Command_Window.set_text_color(2, "White");
            else
        ...
        protected virtual void update_unit_window()
        {
            UnitWindow.update(Active && ready && (Trading || !Unit_Selected));
        }
        protected virtual void update_command_window()
        {
        ...
        // Update input modes
        new internal PrepItemsInputResults update_input()
        {
            #region Trading
            if (this.trading)
            {
                ...
                else
                {
                    ...
                    if (selected_index.IsSomething)
                    {
                        if (confirm_trade())
                        {
                            ...
                            this.active = false;
                            return PrepItemsInputResults.OpenTrade;
                        }
                        else
                        {
                            Global.game_system.play_se(System_Sounds.Buzzer);
                            return PrepItemsInputResults.None;
                        }
                    }
                    ...
                    if (status_index.IsSomething)
                    {
                        Global.game_system.play_se(System_Sounds.Confirm);
                        this.active = false;
                        return PrepItemsInputResults.Status;
                    }
                }
            }
            #endregion
        ...
        protected virtual PrepItemsInputResults update_selected_input()
        {
                ...
                switch (Command_Window.selected_index())
                {
                    ...
                    case 4:
                        if (Global.battalion.convoy_id > -1)
                        {
                            Global.game_system.play_se(System_Sounds.Confirm);
                            this.active = false;
                            return PrepItemsInputResults.Supply;
                        }
                        else
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        break;
                    ...
                    // Shop
                    case 6:
                        if (Global.battalion.convoy_id > -1 && Global.game_battalions.active_convoy_shop != null)
                        {
                            Global.game_system.play_se(System_Sounds.Confirm);
                            Global.game_system.Shopper_Id = this.actor_id;
                            this.active = false;
                            return PrepItemsInputResults.Shop;
                        }
                        else
                            Global.game_system.play_se(System_Sounds.Buzzer);
                        break;
        ...
        private PrepItemsInputResults update_base_input()
        {
            // Close this window
            if (Global.Input.triggered(Inputs.B))
            {
                Global.game_system.play_se(System_Sounds.Cancel);
                close();
                return PrepItemsInputResults.Closing;
            }
        ...
        private void use_item()
        {
            ...
            {
                Global.Audio.play_se("System Sounds", "Gain");
                Use_Confirm_Window = null;
                ...
                //if (boosts.Keys.Count > 0) //Debug
                {
                    ...
                    Stats_Popup = new Stat_Boost_Popup(item.Id, !item.is_weapon, Global.game_map.last_added_unit, -1);
        ...
        {
            if (Global.battalion.is_convoy_full || actor.has_no_items)
            {
                Global.game_system.play_se(System_Sounds.Buzzer);
                return;
            }
            while (!Global.battalion.is_convoy_full && actor.num_items > 0)
            {
                Global.game_battalions.add_item_to_convoy(actor.items[actor.num_items - 1]);
