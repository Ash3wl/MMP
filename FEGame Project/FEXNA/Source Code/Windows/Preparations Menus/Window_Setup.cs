...
namespace FEXNA
{
    enum PrepSetupInputResults { None, PickUnits, Trade, Fortune, CheckMap, Save, StartMap }
    ...
    class Window_Setup : Windows.Map.Map_Window_Base
    {
        ...
        internal Maybe<int> selected_index { get { return CommandWindow.selected_index(); } }
        ...
        {
            Black_Screen_Timer -= Black_Screen_Fade_Timer;
            update();
        }
        ...
        protected virtual void initialize_sprites()
        {
            ...
            // Command Window
            CommandWindow = new Window_Command(new Vector2(8, 32), 80, command_window_string());
        ...
        {
            return new List<string> { "Pick Units", "Items", "About", "Check Map", "Save" };
        }
        ...
        {
            switch ((Preparations_Choices)CommandWindow.index)
            {
                // Pick Units
                case Preparations_Choices.Pick_Units:
                    HelpText.text = Global.system_text["Prep Pick Units"];
                    break;
                // Items
                case Preparations_Choices.Trade:
                    HelpText.text = Global.system_text["Prep Items"];
                    break;
                // Fortune
                case Preparations_Choices.Fortune:
                    if (Global.game_state.augury_event_exists())
                    ...
                    break;
                // Check Map
                case Preparations_Choices.Check_Map:
                    HelpText.text = Global.system_text["Prep Check Map"];
                    break;
                // Save
                case Preparations_Choices.Save:
        ...
        public override void update()
        {
            ...
            CommandWindow.update(Active && ready &&
                !Global.game_system.is_interpreter_running && !Global.scene.is_message_window_active &&
        ...
        new internal PrepSetupInputResults update_input()
        {
            if (CommandWindow.is_selected())
            {
                switch ((Preparations_Choices)(int)CommandWindow.selected_index())
                {
                    // Pick Units
                    case Preparations_Choices.Pick_Units:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        return PrepSetupInputResults.PickUnits;
                    // Trade
                    case Preparations_Choices.Trade:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        return PrepSetupInputResults.Trade;
                    // Fortune
                    case Preparations_Choices.Fortune:
                        return PrepSetupInputResults.Fortune;
                    // Check Map
                    case Preparations_Choices.Check_Map:
                        Global.game_system.play_se(System_Sounds.Confirm);
                        return PrepSetupInputResults.CheckMap;
                    // Save
                    case Preparations_Choices.Save:
                        return PrepSetupInputResults.Save;
                }
            }
            else if (CommandWindow.is_canceled())
            {
                ...
                start_ui_button_pressed)
            {
                Global.game_system.play_se(System_Sounds.Confirm);
                return PrepSetupInputResults.StartMap;
            }
            return PrepSetupInputResults.None;
        }
        ...
        {
            if (!Config.PREP_MAP_BACKGROUND && Background != null)
            {
                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
