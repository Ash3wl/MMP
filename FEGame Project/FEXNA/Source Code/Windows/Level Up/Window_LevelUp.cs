    ...
    internal class Window_LevelUp : Sprite
    {
        ...
        protected void initialize()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                Stat_Labels[i].offset += new Vector2(-(0 + (i / 4) * 64), -((i % 4) * 16));
            }
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                Stats[i].offset += new Vector2(-(48 + (i / 4) * 64), -((i % 4) * 16));
            }
            // Face
        ...
        {
            bool ready = Stat == -1 && Swirls.Count == 0;
            if (ready)
            {
                if (Skip_Wait_Time <= 0)
                    return true;
        ...
        protected virtual void refresh_stat_label_color(int i)
        {
            if (Constants.Actor.DISPLAY_STAT_AVERAGES &&
                (!Constants.Actor.ONLY_PC_AVERAGES || unit.is_player_team))
            {
        ...
        protected virtual void gain_stat(int i, int stat)
        {
            actor.gain_stat(i, stat);
        }
            ...
            switch (Timer)
            {
                case 0:
                    switch (Stat)
                    {
                        ...
                        case 7:
                        case 8:
                            Stat_Gain = Stat_Array[Stat - 1];
                            Stat_Array[Stat - 1] = 0;
                            gain_stat(Stat - 1, Stat_Gain);
                            if (Skipping)
                                stat_sound = true;
                            else
                                Global.game_system.play_se(System_Sounds.Level_Up_Stat);
                ...
                case 1:
                    ...
                    switch (Stat)
                    {
                        ...
                        case 7:
                        case 8:
                            Swirls[Swirls.Count - 1].loc = loc +
                                new Vector2(16 + (((Stat - 1) / 4) * 64), -16 + (((Stat - 1) % 4) * 16)) + new Vector2(0, -2);
                            Arrows.Add(new Stat_Change_Arrow());
                            Arrows[Arrows.Count - 1].texture = Arrow_Texture;
                            Arrows[Arrows.Count - 1].loc = loc +
                                new Vector2(56 + (((Stat - 1) / 4) * 64), -8 + (((Stat - 1) % 4) * 16)) + new Vector2(-3, -2);
                            if (Stat_Gain > 0)
                            {
                                Bars.Add(new Stat_Change_Bar());
                                Bars[Bars.Count - 1].texture = Bar_Texture;
                                Bars[Bars.Count - 1].loc = loc +
                                    new Vector2(12 + (((Stat - 1) / 4) * 64), 12 + (((Stat - 1) % 4) * 16)) + new Vector2(-2, -3);
                            }
                            break;
                    }
                    // Effects
                ...
                case 15:
                    if (Stat > 0)
                    {
                        ...
                        Stat_Gains[Stat_Gains.Count - 1].loc = loc +
                            new Vector2(64 + (((Stat - 1) / 4) * 64), 14 + (((Stat - 1) % 4) * 16)) + new Vector2(-3, -3);
                    }
                    // Numbers
                    break;
                case 17:
        ...
        {
            Stat_Array = actor.full_level_up();
        }
