...
namespace FEXNA.Windows.Map
{
    ...
    internal class Window_Unit : Map_Window_Base
    {
        ...
        readonly static KeyValuePair<int, string>[][] HEADERS = new KeyValuePair<int, string>[][] {
            ...
            new KeyValuePair<int, string>[]{
                new KeyValuePair<int, string>(16, "Equip"), new KeyValuePair<int, string>(96, "Skills")},
            new KeyValuePair<int, string>[]{
        ...
        protected virtual void initialize()
        {
            ...
            Preparations = Global.game_system.preparations;
            Team = determine_team();
        ...
        protected virtual List<int> determine_team()
        {
            ...
            if (Preparations || Global.scene.is_worldmap_scene)
            {
        ...
        {
            var stats_a = new BattlerStats(unit_a.id);
            var stats_b = new BattlerStats(unit_b.id);
            ...
            switch ((Unit_Sort)new_sort)
            {
                ...
                case Unit_Sort.Equip:
                    ...
                    {
                        value = (int)Global.data_weapons[weapon_id_b].Rank - (int)Global.data_weapons[weapon_id_a].Rank;
                        // If the rank is the same, go by type
                        if (value == 0)
                        {
                            value = (int)Global.data_weapons[weapon_id_a].Main_Type - (int)Global.data_weapons[weapon_id_b].Main_Type;
                            // If the type is the same, use the price/use
                            if (value == 0)
                            {
                                value = (int)Global.data_weapons[weapon_id_a].Cost - (int)Global.data_weapons[weapon_id_b].Cost;
        ...
        {
            if (Delay == 0 && !Closing && Black_Screen_Timer <= 0)
            {
                // Change page
                ...
                else if ((Preparations && Global.Input.triggered(Inputs.A) && Row > 0) && !is_help_active)
                {
                    ...
                    Global.Input.gesture_triggered(TouchGestures.LongPress))) //Debug
                {
                    ...
                    Unit_Selected = Global.Input.triggered(Inputs.A);
                    if (Unit_Selected)
