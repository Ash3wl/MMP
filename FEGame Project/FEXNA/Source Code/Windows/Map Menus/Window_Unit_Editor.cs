...
namespace FEXNA.Windows.Map
{
    class Window_Unit_Editor : Map_Window_Base
    {
        ...
        protected virtual void refresh()
        {
            ...
            if (test_battler.Generic)
            {
                ...
                for (int i = 0; i < Constants.Actor.NUM_ITEMS; i++)
                {
                    ...
                    {
                        if (Global.data_weapons.ContainsKey(test_battler.Items[i].Id))
                            item = test_battler.Items[i].to_equipment;
                    }
                    else
                    {
                        if (Global.data_items.ContainsKey(test_battler.Items[i].Id))
                            item = test_battler.Items[i].to_equipment;
                    }
                    if (item == null)
                    {
                    ...
                    {
                        if (test_battler.Items[i].is_weapon && !actor.is_equippable(Global.data_weapons[test_battler.Items[i].Id]))
                            Data[item_index].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Grey");
                        try
                        {
                    ...
                    Data[item_index].text = item == null ? "None" : item.Name;
                }
            }
            else
            {
        ...
        {
            if (!Closing && Black_Screen_Timer <= 0 && Active)
            {
                if (Option_Selected)
                {
        ...
        protected void setup_actor(Test_Battle_Character_Data test_battler)
        {
            ...
            if (test_battler.Generic)
            {
                if (Reinforcement && Global.game_actors.ContainsKey(test_battler.Actor_Id) &&
                        Global.game_actors.is_temp_actor(Global.game_actors[test_battler.Actor_Id]))
                {
        ...
        protected virtual void update_option_change(bool right, bool trigger)
        {
            ...
            switch (Index)
            {
                ...
                // Name
                case 9:
                    int name_index = Config.UNIT_EDITOR_NAMES.IndexOf(test_battler.Name);
                    test_battler.Name = name_index == -1 ? Config.UNIT_EDITOR_NAMES[0] : Config.UNIT_EDITOR_NAMES[(name_index + (right ? 1 : -1) + Config.UNIT_EDITOR_NAMES.Count) % Config.UNIT_EDITOR_NAMES.Count];
                    break;
                // Con
                case 10:
        ...
        protected void update_inventory()
        {
            ...
            {
                actor.items[i] = this.test_battler.Items[i];
            }
