...
namespace FEXNA
{
    abstract class Window_Business
    {
        ...
        protected void set_images()
        {
            ...
            if (Global.game_system.preparations && string.IsNullOrEmpty(Shop.face))
            {
                ...
                if (Global.battalion.convoy_id > 0)
                    (Face as Miniface).set_actor(Global.game_actors[Global.battalion.convoy_id].face_name);
                else
                    (Face as Miniface).set_actor("Convoy");
        ...
        public void draw(SpriteBatch sprite_batch)
        {
            if (Visible)
            {
                ...
                Darkened_Bar.draw(sprite_batch);
                sprite_batch.End();

                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
                Portrait_Bg.draw(sprite_batch);
                if (Portrait_Label != null)
                    Portrait_Label.draw(sprite_batch);
                sprite_batch.End();
                ...
                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
                Gold_Window.draw(sprite_batch);
                Gold_Data.draw(sprite_batch);
                Gold_G.draw(sprite_batch);
                sprite_batch.End();
                sprite_batch.GraphicsDevice.ScissorRectangle = Scene_Map.fix_rect_to_screen(Item_Rect);
                sprite_batch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, null, null, Item_State);
