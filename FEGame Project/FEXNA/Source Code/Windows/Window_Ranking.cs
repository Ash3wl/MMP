...
namespace FEXNA
{
    class Window_Ranking
    {
        ...
        private int Turns, Combat, Exp, Completion, Total;
        ...
        public Window_Ranking(Game_Ranking ranking)
        {
            ...
            Window_Img.width = 154;//120;
//#if DEBUG
            //Window_Img.width = 144;
//#endif
            Window_Img.set_lines(4);
            Window_Img.loc = new Vector2(320, 96);//(Config.WINDOW_WIDTH - Window_Img.width) / 2, (Config.WINDOW_HEIGHT - 0) / 2);
            Window_Img.stereoscopic = Config.RANKING_WINDOW_DEPTH;
            Text = new List<FE_Text>();
            for (int i = 0; i < 4; i++)
            {
                Text.Add(new FE_Text());
                Text[Text.Count - 1].loc = new Vector2(8, 8 + i * 16);
                Text[Text.Count - 1].Font = "FE7_Text";
                Text[Text.Count - 1].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Yellow");
                Text[Text.Count - 1].stereoscopic = Config.RANKING_WINDOW_DEPTH;
            }
            Text[0].text = "Turns Taken";
            Text[1].text = "Damage Taken";
            Text[2].text = "Exp Gained";
            Text[3].text = "Completion Gained";
            //Text[3].text = "MVP";
            for (int i = 0; i < 4; i++)
            {
                Text.Add(new FE_Text_Int());
                Text[Text.Count - 1].loc = new Vector2(Window_Img.width - 8, 8 + i * 16);
                Text[Text.Count - 1].Font = "FE7_Text";
                Text[Text.Count - 1].texture = Global.Content.Load<Texture2D>(@"Graphics/Fonts/FE7_Text_Blue");
                Text[Text.Count - 1].stereoscopic = Config.RANKING_WINDOW_DEPTH;
            }
            Text[4].text = "0";
            Text[5].text = "0";
                    ...
                    switch (Timer)
                    {
                        case 0:
                            if (this.skip_input_triggered)
                            {
                                ...
                                Playing_Sound = false;
                                Text[4].text = Ranking.turns_letter + " " + Ranking.turns_display.ToString() + " / " + Ranking.Data.OutOfTurns;
                                Text[5].text = Ranking.combat_letter + " " + Ranking.combat_display.ToString() + " / " + Ranking.Data.OutOfCombat;
                                Text[6].text = Ranking.exp_letter + " " + Ranking.exp_display.ToString() + " / " + Ranking.Data.OutOfExp;
                                Text[7].text = Ranking.completion_letter + " " + Ranking.completion_display.ToString() + " / " + Ranking.Data.OutOfCompletion;
                                Timer++;
                            }
                            else
                            {
                                bool ready = true;
                                if (Turns < Ranking.turns_display)
                                {
                                    Turns++;
                                    Text[4].text = Turns.ToString() + " / " + Ranking.Data.OutOfTurns;
                                    ready = false;
                                }
                                else if (Turns == Ranking.turns_display && !Text[4].text.Any(x => char.IsLetter(x)))
                                {
                                    
                                    Text[4].text = Ranking.turns_letter + " " + Text[4].text;
                                }
                                if (Combat < Ranking.combat_display)
                                {
                                    Combat++;
                                    Text[5].text = Combat.ToString() + " / " + Ranking.Data.OutOfCombat;
                                    ready = false;
                                }
                                else if (Combat == Ranking.combat_display && !Text[5].text.Any(x => char.IsLetter(x)))
                                {
                                    Text[5].text = Ranking.combat_letter + " " + Text[5].text;
                                }
                                if (Exp < Ranking.exp_display)
                                {
                                    Exp++;
                                    Text[6].text = Exp.ToString() + " / " + Ranking.Data.OutOfExp;
                                    ready = false;
                                }
                                else if (Exp == Ranking.exp_display && !Text[6].text.Any(x => char.IsLetter(x)))
                                {
                                    Text[6].text = Ranking.exp_letter + " " + Text[6].text;
                                }
                                if (Completion < Ranking.completion_display)
                                {
                                    Completion++;
                                    Text[7].text = Completion.ToString() + " / " + Ranking.Data.OutOfCompletion;
                                    ready = false;
                                }
                                else if (Completion == Ranking.completion_display && !Text[7].text.Any(x => char.IsLetter(x)))
                                {
                                    Text[7].text = Ranking.completion_letter + " " + Text[7].text;
                                }
                                if (ready)
                                {
                                    Global.game_system.cancel_sound();
                    ...
                    switch (Timer)
                    {
                        case 0:
                            if (this.skip_input_triggered)
                            {
                                /*Global.game_system.cancel_sound();
                                Playing_Sound = false;
                                Text[7].text = Ranking.score.ToString();*/
                                Timer++;
                            }
                            else
                            {
                ...
                case 9:
                    switch (Timer)
                    {
                        case 30:
                            ...
                            break;
                        default:
                            Rank.scale = new Vector2(1f + 5f * (30 - Timer) / 30f);
