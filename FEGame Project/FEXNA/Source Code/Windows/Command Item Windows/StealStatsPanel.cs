...
namespace FEXNA.Windows.Command.Items
{
    class StealStatsPanel : StatsPanel
    {
            ...
            : base(actor)
        {
            ...
            Rate = new FE_Text();
            Rate.loc = FaceBg.loc + new Vector2(64, 96);
            Rate.Font = "FE7_Text";
            Rate.texture = Global.Content.Load<Texture2D>(
                @"Graphics/Fonts/FE7_Text_White");
            Rate.text += "STEAL CHANCE GOES HERE";
            Rate.offset = new Vector2((int)(Font_Data.text_width(Rate.text) * 0.4f), 0);
        }
        ...
        public override void Draw(SpriteBatch sprite_batch)
        {
            ...
            Rate.draw(sprite_batch, -data_draw_offset);
            sprite_batch.End();
            ...
            if (!(texture == null))
                if (visible)
                {
                    ...
                    sprite_batch.Draw(texture, (loc + draw_vector()) - draw_offset, new Rectangle(0, 0, 80, HEADER_HEIGHT),
                            ...
                            mirrored ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Z);
                    //draw footer
                    sprite_batch.Draw(texture, (loc + draw_vector()) - draw_offset - new Vector2(-16,-96), new Rectangle(0, 72, 80, HEADER_HEIGHT),
                            tint, angle, offset - Vector2.Zero, scale,
                            mirrored ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Z);
                }
        }
