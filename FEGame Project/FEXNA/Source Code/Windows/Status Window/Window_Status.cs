...
namespace FEXNA
{
    partial class Window_Status
    {
        ...
        internal int current_unit { get { return Team[Unit_Id]; } }
        ...
        internal static bool show_stat_averages(Stat_Labels stat_label)
        {
            if (Constants.Actor.DISPLAY_STAT_AVERAGES)
                if ((!Constants.Actor.STAT_AVERAGES_ONLY_IN_PREP ||
                        (Global.scene is Scene_Worldmap ||
                            Global.game_system.preparations)) &&
                        stat_label < Stat_Labels.Con)
                    return true;
            return false;
        }
        ...
        public void update()
        {
            ...
            update_black_screen();
            // Inputs
            bool input = Delay == 0 && !Closing && Black_Screen_Timer <= 0;
        ...
        {
            Left_Page_Arrow.update_input(new Vector2(0, Offset.Y));
            Right_Page_Arrow.update_input(new Vector2(0, Offset.Y));
            if (Delay != 0)
                return;
        ...
        {
            if (!Help_Data[page].ContainsKey(index))
                reset_help_index();
            else
                Help_Index = index;
            //Help_Data[page][index].Loc;
            switch (Help_Data[page][Help_Index].Data)
            {
                case "Name":
#if DEBUG
                    string identifier = "";
                    ...
                    Help_Window.set_text(unit.actor.description + "\n" + unit.id.ToString() + ", " + identifier);
#else
                    Help_Window.set_text(unit.actor.description);
#endif
                    break;
                ...
                case "Pow":
                    ...
                    break;
                case "Item":
                    Help_Window.set_item(unit.actor.items[Convert.ToInt32(Help_Index.Substring(4, 1)) - 1], unit.actor);
                    break;
                case "Skill":
                    Help_Window.set_text(Global.data_skills[unit.actor.skills[
                        Convert.ToInt32(Help_Index.Substring(5, 1)) - 1]].Description.Replace("|", "\n"));
                    break;
                case "Status":
                    Help_Window.set_text(Global.data_statuses[unit.actor.states[
                        Convert.ToInt32(Help_Index.Substring(6, 1)) - 1]].Description.Replace("|", "\n"));
                    break;
                default:
                    if (Help_Text_Keys.Contains(Help_Data[page][Help_Index].Data))
                        Help_Window.set_text(Global.system_text["Status Help " + Help_Data[page][Help_Index].Data]);
                    else
                        Help_Window.set_text(Help_Data[page][Help_Index].Data);
                    break;
            }
