...
namespace FEXNA
{
    ...
    public class Window_Message : Stereoscopic_Graphic_Object
    {
        ...
        public readonly static Dictionary<int, int> TEXT_SPEED = new Dictionary<int, int> {
            { (int)Message_Speeds.Slow,   9 }, // Slow
            { (int)Message_Speeds.Normal, 5 }, // Norm
            { (int)Message_Speeds.Fast,   2 }, // Fast
            { (int)Message_Speeds.Max,    0 }  // Max
        };
        protected const int SCROLL_TIME = 16;
        const int ARROW_WIDTH = 12;

        protected int Message_Lines = Face_Sprite_Data.MESSAGE_LINES;
        protected int Text_Speed;
...
#if DEBUG
            get { return (int)(Math.Pow(2, Pitch) * 100); }
#endif
        }
...
function normally. Suggested value is -3.",
            ...
            Text_Speed = Global.game_options.text_speed;
            reset(false, new Vector2(40, 24), 240, Message_Lines * 16 + 16);
        }
        ...
        protected void new_text_box()
        {
            ...
            reset_text_color();
            Window_Img.try_set_speaker(Speaker, Widths[0], Faces.ContainsKey(Speaker) ? (Maybe<int>)Faces[Speaker].loc.X : default(Maybe<int>), current_speaker_name);
        }
        ...
        {
            int new_width = Font_Data.text_width(copy_text2, "FE7_Convo") + offset;
        ...
        protected bool text_process()
        {
            if (Line_Wait <= 0)
            {
                ...
                if (!Scroll && Line_Wait <= 0)
                {
                    ...
                    if (Text.Count == 0)
                        return false;
                    if (Timer >= TEXT_SPEED[Text_Speed]) Timer = 0;
        ...
        private Maybe<bool> process_next_character()
        {
            ...
            Text_Loc.X += Font_Data.text_width("" + current_character, "FE7_Convo");
                ...
                if (text_command.StartupCommand)
                {
                    ...
                    switch (key)
                    {
                        ...
                        case "E":
                            ...
                            if (Text_Speed != (int)Message_Speeds.Max || Phase == 0)
                                refresh();
                            return false;
                        #endregion
                        ...
                        case "R":
                            ...
                            return (Text_Speed == (int)Message_Speeds.Max && Phase != 0);
                        #endregion
                        ...
                        case "Music":
                            ...
                            if (test_text != "nil")
                                Global.Audio.play_bgm(test_text);
                            return false;
                        #endregion
                ...
                else
                {
                    switch (key)
                    {
                        ...
                        case "M":
                            ...
                            Faces.Remove(id1);
                            Faces.Remove(id2);
                            if (face1 != null) Faces[id2] = face1;
                            if (face2 != null) Faces[id1] = face2;
                            // Reorder names
                            string temp_name = Names[id1];
                            Names[id1] = Names[id2];
                            Names[id2] = temp_name;
                            ...
                            if (Text_Speed != (int)Message_Speeds.Max)
                                refresh();
                            return (Text_Speed == (int)Message_Speeds.Max && Phase != 0);
                        #endregion
                        #region W, Wait: Wait
                        ...
                        case "Wait":
                            ...
                            if (!skip)
                            {
                                if (Text_Speed == (int)Message_Speeds.Max && Text_Shown)
                        ...
                        case "X":
                            pitch = (int)MathHelper.Clamp(Convert.ToInt32(test_text), 1, 400);
                            return true;
                        #endregion
        ...
        private static void get_face_name(string raw_name, out string filename, out string recolor_country)
        {
            ...
            switch (raw_name)
            {
                ...
                case "Visitor":
                    ...
                    {
#endif
                        filename = Global.game_state.event_caller_unit.actor.face_name;
                        if (Global.game_state.event_caller_unit.actor.generic_face)
                            recolor_country = Global.game_state.event_caller_unit.actor.name_full;
#if DEBUG
                    }
#endif
                    break;
                ...
                case "Selected":
                    ...
                    {
#endif
                        filename = Global.game_map.units[Global.game_system.Selected_Unit_Id].actor.face_name;
                        if (Global.game_map.units[Global.game_system.Selected_Unit_Id].actor.generic_face)
                            recolor_country = Global.game_map.units[Global.game_system.Selected_Unit_Id].actor.name_full;
#if DEBUG
                    }
#endif
                    break;
                ...
                case "Enemy":
                    ...
                    {
                        filename = enemy.actor.face_name;
                        if (enemy.actor.generic_face)
                            recolor_country = enemy.actor.name_full;
                    }
                    break;
                default:
                    filename = raw_name;
        ...
        {
            if (valid_face_speaker() && Faces.ContainsKey(Speaker) && Text_Speed != (int)Message_Speeds.Max)
                Faces[Speaker].talk();
        }
        ...
        protected virtual void create_arrow()
        {
            ...
            float y = this.loc.Y + Text_Loc.Y * 16 + 8;
            ...
            else if (Active)
            {
                ...
                if (Text == null || !Wait)
                {
                    ...
                    if (Wait_Timer > 0)
                    {
                        Text_Shown = false;
                        Wait_Timer--;
                        if (Wait_Timer != 0 && !(Global.Input.triggered(Inputs.B) || Global.Input.triggered(Inputs.Start)) || Phase == 0)
                        {
                            update_faces();
                            return;
                        }
                    }
                    else if (Phase != 2 || Text_Speed != (int)Message_Speeds.Max)
                        refresh();
                }
        ...
        protected virtual bool input_update()
        {
            ...
            {
                // Max text speed
                if (Text_Speed == (int)Message_Speeds.Max && !Wait)
                {
                    Skipping = true;
                    for (; ; )
                    {
