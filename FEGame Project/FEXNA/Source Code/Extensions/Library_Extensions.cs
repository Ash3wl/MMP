...
namespace FEXNAWeaponExtension
{
    public static class Extension
    {
            ...
            bool halveOnHealingTerrain = true)
        {
            ...
            //new Item_Data(Item_Data_Type.Weapon, 1, -1)
            Item_Data ring = new Item_Data(Item_Data_Type.Item, 6);
            if (target != null && target.actor.id == 34 && unit.actor.has_ring)
                effectiveness = 3;

            if (target != null && unit.actor.has_skill("PHANTO"))
            {
                switch (target.actor.class_id){
                    case 93:
                    case 88:
                    case 123:
                    case 124:
