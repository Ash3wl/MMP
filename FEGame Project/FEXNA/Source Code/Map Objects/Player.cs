...
namespace FEXNA
{
    internal class Player : Map_Object
    {
        ...
        protected int Dir8_Last_Frame = 0, Move_Timer = 0, Move_Sound_Timer = 0;
        ...
        public void read(BinaryReader reader)
        {
            ...
            Dir8_Last_Frame = reader.ReadInt32();
        ...
        protected void update_input()
        {
            ...
            int dir8 = Global.Input.dir8();
            if ((dir8 != 0 || !is_on_square) && Global.game_temp.minimap_call)
            {
                if (Loc != center_cursor_loc(false))
                    not_minimap_centered = true;
            }
        ...
        protected void range_edge_check()
        {
            if (Global.game_system.Selected_Unit_Id == -1)
                return;
            if (!Global.game_map.active_team.Contains(Global.game_system.Selected_Unit_Id))
                return; //Multi
            Vector2 test_vector = Vector2.Zero;
            List<Inputs> locked_inputs = new List<Inputs>();
            switch (Global.Input.dir8())
            {
                // Down left
                case 1: test_vector = new Vector2(is_left_edge() ? 0 : -1, is_bottom_edge() ? 0 : 1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Down, Inputs.Left }); break;
                // Down
                case 2: test_vector = new Vector2(0, 1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Down }); break;
                // Down right
                case 3: test_vector = new Vector2(is_right_edge() ? 0 : 1, is_bottom_edge() ? 0 : 1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Down, Inputs.Right }); break;
                // Left
                case 4: test_vector = new Vector2(-1, 0);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Left }); break;
                // Right
                case 6: test_vector = new Vector2(1, 0);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Right }); break;
                // Up left
                case 7: test_vector = new Vector2(is_left_edge() ? 0 : -1, is_on_top() ? 0 : -1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Up, Inputs.Left }); break;
                // Up
                case 8: test_vector = new Vector2(0, -1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Up }); break;
                // Up right
                case 9: test_vector = new Vector2(is_right_edge() ? 0 : 1, is_top_edge() ? 0 : -1);
                    locked_inputs.AddRange(new List<Inputs> { Inputs.Up, Inputs.Right }); break;
            }
            if (!Global.game_map.move_range.Contains(Loc + test_vector) && Global.game_map.move_range.Contains(Loc))
            {
                foreach (Inputs key in locked_inputs)
                    Input.lock_repeat(key);
            }
        }
