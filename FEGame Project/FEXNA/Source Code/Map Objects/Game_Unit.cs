...
namespace FEXNA
{
    ...
    internal partial class Game_Unit : Combat_Map_Object
    {
        ...
        public bool Cantoforcing = false; //i am bad practice
        protected int cantoforce_used = 0;
        ...
        public List<Item_Data> items
        {
            get
            {
                ...
                if (can_use_siege() && Loc != Config.OFF_MAP && !Gladiator)
                {
                    ...
                    if (siege != null && siege.is_ready && actor.is_equippable(Global.data_weapons[siege.item.Id]))
        ...
        public int canto_mov
        {
            get
            {
                ...
                if (Cantoforcing)
                    mov = Math.Max(0, Math.Min(mov, mov - (Moved_So_Far + cantoforce_used)));//lawl what is this
                    //mov -= Moved_So_Far + cantoforce_used;
                if (Cantoing)
                    mov -= Moved_So_Far;
                return mov;
            }
        ...
        internal Maybe<int> terrain_def_bonus(Game_Unit target)
        {
            ...
            if (target != null && !nihil(target))
                if (actor.has_skill("CMNDO"))
                    result = (result.IsNothing ? 0 : (int)result) + (terrain_def_bonus() / 2);
            // If terrain is ignored, return the result so far
            if (target != null && ignore_terrain_def())
                return result;
            return (result.IsNothing ? 0 : (int)result) + terrain_def_bonus();
        }
        ...
        internal Maybe<int> terrain_res_bonus(Game_Unit target)
        {
            ...
            if (target != null && !nihil(target))
                if (actor.has_skill("CMNDO"))
                    result = (result.IsNothing ? 0 : (int)result) + (terrain_res_bonus() / 2);
            // If terrain is ignored, return the result so far
            if (ignore_terrain_def())
                //if (target != null && ignore_terrain_def()) //Debug
                return result;
            return (result.IsNothing ? 0 : (int)result) + terrain_res_bonus();
        }
        ...
        internal Maybe<int> terrain_avo_bonus(Game_Unit target)
        {
            Maybe<int> result = default(Maybe<int>);
            // Skills: Commando
            if (target != null && !nihil(target))
                if (actor.has_skill("CMNDO"))
                    result = (result.IsNothing ? 0 : (int)result) + (terrain_avo_bonus() / 2);
            // If terrain is ignored, return the result so far
            if (ignore_terrain_avo())
                //if (target != null && ignore_terrain_avo()) //Debug
                return result;
            return (result.IsNothing ? 0 : (int)result) + terrain_avo_bonus();
        }
        ...
        {
            return (weapon.is_staff() || weapon.is_always_magic() || (distance > 1 && weapon.is_ranged_magic()));
        }

        #region Items
        public void equip(int index)
        {
            if (index < Constants.Actor.NUM_ITEMS + 1)
                actor.equip(index);
            else
            {
                actor.weapon_id = items[index - 1].Id;
            }
        }

        protected void discard_item(int index)
        {
            if (index < Constants.Actor.NUM_ITEMS)
                actor.discard_item(index);
        }
        ...
        public bool is_weapon_broke(int uses)
        {
            ...
            if (weapon.Uses <= 0) return false;
            if (Using_Siege_Engine)
                return Constants.Actor.NUM_ITEMS >= items.Count ||
                    items[Using_Siege_Engine ? Constants.Actor.NUM_ITEMS : actor.equipped - 1].Uses - uses <= 0;
            else
                return items[actor.equipped - 1].Uses - uses <= 0;
        }
        ...
        public bool can_acquire_drops
        {
            get
            {
                if (Constants.Gameplay.CITIZENS_GET_DROPS &&
                        !is_attackable_team(Constants.Team.PLAYER_TEAM) &&
        ...
        public int vision()
        {
            ...
            return vision_skill(base_range, Vision_Bonus);
        }
        ...
        public bool visible_by(int team)
        {
        ...
        public bool can_canto_move()
        {
            ...
            Cantoing = true;
            HashSet<Vector2> locs = Pathfinding.get_range(Loc, canto_mov, Id);
            Cantoing = cantoing;
            locs.Remove(Loc);
            return locs.Count > 0;
        }
        ...
        public int dance_exp()
        {
            ...
            actor.exp += Constants.Combat.DANCE_EXP;
            return exp_gain;
        }
        ...
        public int weapon_use_count(bool is_hit)
        {
            ...
            if ((actor.has_skill("ANGLE") && actor.weapon.type != "Fluff"))
                return 0;
            return actor.weapon_use_count(is_hit, is_player_team); // Overwritten by Astra, Thaumaturgy, and Multishot //Yeti
        }
        ...
        public void weapon_use()
        {
            ...
            else if(!actor.has_skill("YURI") && !(actor.has_skill("ANGLE") && actor.weapon.type != "Fluff"))
                actor.weapon_use();
        }
        ...
        internal bool any_effective_weapons(Game_Unit target)
        {
            ...
            {
                if (item.is_weapon && actor.is_equippable(item.to_weapon) && item.to_weapon.effective_multiplier(this, target) > 1)
                    return true;
            }
            return false;
        }
        ...
        public bool can_counter(Game_Unit attacker, Data_Weapon weapon1, int distance, int item_index)
        {
            ...
            {
                if (!actor.is_equippable(item_index)) return false;
        ...
        {
            foreach (Item_Data data in items)
                if (data.is_weapon && actor.is_equippable(Global.data_weapons[data.Id]) &&
                        Global.data_weapons[data.Id].No_Counter)
                    return true;
            return false;
        }
        ...
        public bool is_opposition
        {
            get
            {
                //return is_attackable_team(Constants.Team.PLAYER_TEAM); //Yeti
                return team == Constants.Team.ENEMY_TEAM ||
                    team == Constants.Team.INTRUDER_TEAM;
            }
        }

        public bool is_passable_team(Game_Unit test_unit)
        {
            if (is_attackable_team(Constants.Team.PLAYER_TEAM)) //!is_player_team) //Debug
            {
            ...
            return !attackable_teams().Contains(test_unit.team);
        }
        ...
        public bool loss_on_death
        {
            get
            {
                return Global.game_system.loss_on_death.Contains(actor.id) || Global.game_system.ally_loss_on_death.Contains(actor.id);
            }
        }
        ...
        public void update_attack_graphics()
        {
        ...
        protected bool is_active_unit_sprite
        {
            get
            {
                return (Global.game_state.new_turn_unit_id == Id && Facing == 6) || (Global.game_state.item_user == this ||
                    Global.game_state.sacrificer_id == Id ||
                    (Global.game_state.staff_active && Global.game_state.battler_1_id == Id)) && !Sprite_Moving; //Yeti
            }
        }
        ...
        private void update_move_range(Vector2 loc, Vector2 baseLoc)
        {
            ...
            Move_Range = new HashSet<Vector2>(
                Pathfinding.get_range(loc, canto_mov, Id, baseLoc)); //HashSet
            ...
            foreach (int id in Global.game_map.units.Keys)
                if (Global.game_map.units[id].visible_by() && Global.game_state.can_talk(Id, id))
                    range.Add(Global.game_map.units[id].loc);
            if (is_player_team)
        ...
        public void update_attack_range()
        {
            ...
            if (can_use_siege())
            {
                ...
                foreach (Siege_Engine siege in Global.game_map.siege_engines.Values)
                    if (siege.is_ready && move_range.Contains(siege.loc) && actor.is_equippable(Global.data_weapons[siege.item.Id]) &&
                        !Global.data_weapons[siege.item.Id].is_staff())
                    {
                        Maybe<int> move_distance = Pathfinding.get_distance(siege.loc, Id, canto_mov, false, Loc);
                        if (move_distance.IsSomething)
                        {
                            set_ai_base_loc(siege.loc, move_distance);

                            // Adds siege engine's range to the attack range
                            int min = min_range(Constants.Actor.NUM_ITEMS);
                            int max = max_range(Constants.Actor.NUM_ITEMS);
        ...
        protected HashSet<Vector2> get_weapon_range(List<int> useable_weapons, HashSet<Vector2> move_range, string skill, bool walls)
        {
            ...
            foreach (int useable_weapon in useable_weapons)
            {
                ...
                if (Global.data_weapons[items[useable_weapon].Id].range_blocked_by_walls() == walls)
                {
        ...
        {
            //wark vandy stealing exceptions
        ...
        public bool can_seize
        {
            get
            {
                return true; //wark everyone can seize lol Constants.Gameplay.MAIN_CHARACTER.Contains(actor.id) || Global.game_map.team_leaders[Team] == Id;
            }
        }
            ...
            foreach (Vector2 offset in new Vector2[] { new Vector2(0, 1), new Vector2(0, -1), new Vector2(1, 0), new Vector2(-1, 0) })
                if (!Global.game_map.is_off_map(offset + Loc))
                    if (!Global.game_map.is_blocked(offset + Loc, Rescuing))
                        if (Pathfinding.passable(Global.game_map.units[Rescuing], offset + Loc))
        ...
        public bool can_rescue(Game_Unit target)
        {
            ...
            if (is_rescuing || is_rescued || actor.has_skill("PURT"))
                return false;
            // Must be allied teams
            if (is_attackable_team(target))
                return false;
            // Don't rescue units that are already themselves rescuing
            if (target.is_rescuing || target.actor.has_skill("PURT") || target.actor.has_skill("HIATUSING"))
                return false;
            return actor.mov > 0 && (aid() >= target.stat(Stat_Labels.Con) || actor.has_skill("SELFLESS"));
        }

        protected override Vector2 real_loc_on_map()
        {
            if (!is_rescued) return Real_Loc;
        ...
        public void escape()
        {
            ...
            Global.game_map.kill_unit(this, false);
            Team = 0;
        }
        ...
        public void set_stat_bonus(Buffs stat, int value)
        {
            Stat_Bonuses[(int)stat] = value < 0 ?
                Math.Min(Stat_Bonuses[(int)stat], value) :
                Math.Max(Stat_Bonuses[(int)stat], value);
        }
        ...
        public bool evented_move_to(Vector2 target_loc, bool ignore_terrain, bool ignore_units = false, float move_speed = -1)
        {
            ...
            bool unit_at_target = !Global.game_map.is_off_map(target_loc) && Global.game_map.get_unit(target_loc) != null;
            // If the target on the map but someone is already there, find the nearest location instead
            if (!ignore_units && unit_at_target)
                target_loc = Pathfinding.find_open_tile(target_loc, Id);
            // If the target is the default off map location, don't move
            if (target_loc == Config.OFF_MAP)
                Evented_Move = false;
            ...
            if (Evented_Move)
            {
                ...
                if (unit_at_target && ignore_units)
                    remove_old_unit_location();
                else
        ...
        protected bool pathfind_move_to(Vector2 target_loc, bool player_move, bool rescue_move = false)
        {
            ...
            List<Vector2> route = Pathfinding.get_route(target_loc, (Evented_Move || rescue_move) ? -1 : mov, Id);
            if (route != null)
            {
                if (Blocked)
                {
                ...
                if (Ai_Move)
                    Temp_Moved += Pathfinding.unit_distance;
                Ai_Move = false;
                Move_Timer = 0;
        ...
        public void done_moving(bool kill_anim = true)//cantoforce
        {
            if(kill_anim)
                Sprite_Moving = false;
            Cantoing = false;
        ...
        private bool move_points_left()
        {
            cantoforce_used++;
            if (actor.has_skill("CANTOFORCE") && cantoforce_used + Moved_So_Far <= mov)
            {
                Cantoforcing = true;
                Global.player.loc = loc;
                Global.player.instant_move = true;
                Global.game_system.Selected_Unit_Id = Id;
                return true;
            }
            else if (cantoforce_used == 99)
            {
        ...
        public void end_cantoforcing()
        {
            cantoforce_used = 99;
        }

        public void new_turn_cantoforce()
        {
        ...
        public void wait(bool update_move_ranges)
        {
            ...
            if (move_points_left())//this stuff also relates to cantoforce
            {
                queue_move_range_update();
                Global.game_map.refresh_move_ranges();
                done_moving(false);
        ...
        public void new_turn()
        {
            if (!Constants.Gameplay.MASTERIES_CHARGE_AT_TURN_END)
                charge_masteries(MASTERY_RATE_NEW_TURN);
            if(actor.id == 10 && !actor.has_skill("ANGLE"))
                charge_masteries_set(20);
            // Don't decrement status timers on the first turn,
            // for statuses applied before the map starts
            if (Global.game_state.turn > 1)
                actor.update_states();
        ...
        protected List<int>[] enemies_in_range(HashSet<Vector2> move_range, int item_index, string skill, bool ai_siege, HashSet<Vector2> enemy_tiles)
        {
            ...
            if (item_index == -1)
                useable_weapons = actor.useable_weapons(ai_siege ? actor.items : items);
            // Otherwise only check the given weapon
            else
            {
                useable_weapons = new List<int>();
                if (actor.useable_weapons(items).Contains(item_index))
            ...
            foreach (int useable_weapon in useable_weapons)
            {
                ...
                result = check_range(min, max, move_range, true, useable_weapon, skill);
                if (enemy_tiles == null)
                {
                    targets.AddRange(result[0]);
                    in_range_weapons.AddRange(result[1]);
                }
                else
                {
                    int i = 0;
                    while (i < result[0].Count)
                        if (!enemy_tiles.Contains(Global.game_map.units[result[0][i]].loc))
                            result[0].RemoveAt(i);
                        else
                            i++;
                    if (result[0].Count > 0)
            ...
            if (ai_siege && can_use_siege())
            {
                ...
                foreach (Siege_Engine siege in Global.game_map.siege_engines.Values)
                    if (siege.is_ready && siege_move_range.Contains(siege.loc) && actor.is_equippable(Global.data_weapons[siege.item.Id]) &&
                        !Global.data_weapons[siege.item.Id].is_staff())
                    {
                        Maybe<int> move_distance = Pathfinding.get_distance(siege.loc, Id, canto_mov, false, Loc);
                        if (move_distance.IsSomething)
                        {
                            ...
                            result = check_range(
                                ...
                                Constants.Actor.NUM_ITEMS, skill);
                            targets.AddRange(result[0]);
        ...
        protected List<int>[] enemies_in_staff_range(HashSet<Vector2> move_range, int item_index, string skill, bool ai_siege)
        {
            ...
            if (item_index == -1)
                useable_staves = actor.useable_attack_staves(ai_siege ? actor.items : items);
            else
            {
                useable_staves = new List<int>();
                if (actor.useable_attack_staves(items).Contains(item_index))
            ...
            foreach (int useable_staff in useable_staves)
            {
                ...
                result = check_range(min, max, move_range, true, useable_staff, skill);
                targets.AddRange(result[0]);
                in_range_weapons.AddRange(result[1]);
            }
            // Siege Engines
        ...
        public List<int>[] allies_in_staff_range(HashSet<Vector2> move_range, int item_index, bool ai_siege)
        {
            List<int> useable_staves;
            if (item_index == -1)
                useable_staves = actor.useable_healing_staves(ai_siege ? actor.items : items);
            else
            {
                useable_staves = new List<int>();
                if (actor.useable_healing_staves(items).Contains(item_index))
                    useable_staves.Add(item_index);
            }
            ...
            foreach (int useable_weapon in useable_staves)
            {
                ...
                result = check_range(min_range, max_range, move_range, false, useable_weapon, "");
                bool added = false;
                foreach (int id in result[0])
                {
                    if (Global.data_weapons[items[useable_weapon].Id].Heals() || Global.data_weapons[items[useable_weapon].Id].Status_Remove.Count > 0)
                    {
        ...
        public List<int>[] untargeted_staff_range(HashSet<Vector2> move_range, int item_index, bool ai_siege)
        {
            List<int> useable_staves;
            if (item_index == -1)
                useable_staves = actor.useable_untargeted_staves(ai_siege ? actor.items : items);
            else
            {
                useable_staves = new List<int>();
                if (actor.useable_untargeted_staves(items).Contains(item_index))
                    useable_staves.Add(item_index);
            }
        ...
        {
            return check_range(min_range, max_range, move_range, attackable, -1, "", objects: objects);
        }
        protected List<int>[] check_range(int min_range, int max_range, HashSet<Vector2> move_range, bool attackable,
            int useable_weapon, string skill, bool objects = true)
        {
            return check_range(min_range, max_range, move_range, attackable, useable_weapon, skill, null, objects: objects);
        }
        protected List<int>[] check_range(int min_range, int max_range, HashSet<Vector2> move_range, bool attackable,
            int useable_weapon, string skill, List<int> units, bool objects = true)
        {
            // If the range is invalid, return
            if (min_range < 1 || max_range < min_range || !move_range.Any())
                return new List<int>[] { new List<int>(), new List<int>() };
            ...
            if (useable_weapon > -1)
            {
                targets = Pathfinding.targets_in_range(min_range, max_range, move_range, units,
                    Global.data_weapons[items[useable_weapon].Id].range_blocked_by_walls());
                if (Global.data_weapons[items[useable_weapon].Id].is_staff())
            ...
            if (skill != "")
            {
                ...
                {
                    if (!valid_mastery_target(skill, Global.game_map.units[targets[i]], Global.game_map.combat_distance(Id, targets[i]),
                            items[useable_weapon].Id))
                        targets.RemoveAt(i);
                    else
                        i++;
                }
            }
            return new List<int>[] { targets, (targets.Count > 0 ? new List<int> { useable_weapon } : new List<int>()) };
        }
        ...
        protected List<int> get_units(bool attackable, bool objects = false, bool healing = false)
        {
            ...
            {
                foreach (int id in Global.game_map.destroyable_objects.Keys)
                {
                    units.Add(id);
                }
            }
        ...
        internal HashSet<int> spell_animation_ids()
        {
            ...
            if (actor.weapon != null && actor.weapon.has_anima_start())
            {
            ...
            // All that other mess
            result.UnionWith(FE_Battler_Image_Wrapper.spell_attack_animation_value(actor.weapon_id, 1, true));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_attack_animation_value_bg1(actor.weapon_id, 1, true));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_attack_animation_value_bg2(actor.weapon_id, 1, true));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_attack_animation_value_fg(actor.weapon_id, 1, true));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_end_animation_value(actor.weapon_id, true));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_lifedrain_animation_value(actor.weapon_id, 1));
            result.UnionWith(FE_Battler_Image_Wrapper.spell_lifedrain_end_animation_value(actor.weapon_id));
            return result;
        }
        ...
        internal HashSet<string> animation_graphics(HashSet<int> animation_ids, bool added_animations = false, bool only_added_animations = true)
        {
            ...
            foreach (int animation_id in animation_ids)
            {
                ...
                if (added_animations)
                {
                    Dictionary<int, List<int>> added_effects = On_Hit.ADDED_EFFECTS(animation_id);
                    if (added_effects != null)
                        foreach (var added_effect in added_effects.Values)
                        {
