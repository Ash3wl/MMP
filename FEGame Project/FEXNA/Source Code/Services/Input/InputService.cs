    ...
    class InputService : BaseInputService
    {
            ...
            Vector2 releasedLoc)
        {
            ...
            for (int i = DeferredTaps.Count - 1; i >= 0; i--)
            {
                ...
                if ((TouchPanelState.CurrentTimestamp -
                        sample.Timestamp).TotalMilliseconds >
                        TouchPanelState.DoubleTapTimeThreshold)
                {
            ...
            bool consumeInput)
        {
            ...
            bool result = transform_rect_contains(
                rect, loc, rectOriginOffset, angle, mirrored, touch_loc);
