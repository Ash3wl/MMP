...
namespace FEXNA
{
    partial class Event_Processor
    {
        ...
        private int process_number(string str, bool allow_unit)
        {
            ...
            {
#if DEBUG
                Debug.Assert(Global.battalion.deployed.Count > 0);
#endif
...
#endif
        {
            ...
            double num_test;
            if (double.TryParse(str, System.Globalization.NumberStyles.Any, System.Globalization.NumberFormatInfo.InvariantInfo, out num_test))
                return Convert.ToInt32(str);
            else
            {
#if DEBUG
        ...
        /*private Game_Actor get_actor(bool from_unit_flag, int id) //Debug
        {
            if (from_unit_flag)
            {
                if (id == -1)
                    if (Global.game_map.last_added_unit != null)
                        return Global.game_map.last_added_unit.actor;
                if (Global.game_map.units.ContainsKey(id))
                    return Global.game_map.units[id].actor;
            }
            else
            {
                if (Global.game_actors.ContainsKey(id))
                    return Global.game_actors[id];
            }
            return null;
        }*/

        private Game_Actor get_actor(bool from_unit_flag, string id_str)
        {
            // Get unit, and then get that unit's actor
            if (from_unit_flag)
            {
#if DEBUG
                if (id_str != "-1" && id_str != "Last_Added_Unit")
                { } //Debug
#endif
            ...
            {
                int id = process_number(command.Value[1]);
                if (Global.game_actors.ContainsKey(id))
                    return Global.game_actors[id];
            }
            return null;
        ...
        private bool command_unit_move()
        {
            ...
            switch (command.Value[0])
            {
                case "Unit to Loc":
                    ...
                    if (unit.loc != target_loc)
                    {
                        bool moved;
                        if (command.Value.Length > 3)
                        {
                            move_speed = process_float(command.Value[3]);
                            moved = unit.evented_move_to(target_loc, false, false, move_speed: move_speed);
                        }
                        else
                            moved = unit.evented_move_to(target_loc, false, false);
                        unit.update();
                        // Move ranges will need to be updated
                        unit_moved = true;
                    }
        ...
        private bool command_death_quote()
        {
            ...
            Game_Actor actor = get_actor(process_bool(command.Value[0]), command.Value[1]);
        ...
        private bool command_unit_theme()
        {
            ...
            if (command.Value[1] == "null")
                Global.game_state.remove_battle_theme(id, command.Value[1]);
            else if (id != -1 && id != 0)
            ...
            if (unit_added)
                switch (command.Value[0])
                {
                    ...
                    case "saved deployment":
                        ...
                        unit_added = Global.battalion.deployed.Count > 0 &&
                            Global.game_map.add_actor_unit(team, loc, Global.battalion.deployed[0], identifier);
                        break;
                    case "temp":
                    ...
                    case "reinforcement":
                        identifier = command.Value.Length < 6 ? "" : command.Value[5];
                        unit_added = Global.game_map.add_reinforcement_unit(team, loc, process_number(command.Value[4]), identifier);
                        break;
                    default:
                        unit_added = false;
                        break;
        ...
        {
            if (command.Value[0] == "Add All Deployed")
            {
        ...
        private bool command_exp()
        {
            ...
            if (actor != null)
            {
                int exp = process_number(command.Value[2]);
                ...
                if (exp != 0)
                {
                    ...
                    if (command.Value.Length >= 5 && process_bool(command.Value[4]) && process_bool(command.Value[0]))
                    {
        ...
        private bool command_change_status()
        {
            ...
            if (actor != null)
            {
                ...
                {
                    if (unit_id != -1)
                        ((Scene_Map)Global.scene).update_map_sprite_status(unit_id);
                    else
        ...
        // Chapter Support Points
        private bool command_chapter_support()
        {
        ...
        private bool command_heal_actors()
        {
            ...
            else
            {
                ...
                {
                    if (process_bool(command.Value[1]))
                        Global.game_actors[process_number(command.Value[0])].hp = process_number(command.Value[2]);
                    else
                        Global.game_actors[process_number(command.Value[0])].hp += process_number(command.Value[2]);
                }
            }
        ...
        private bool command_remove_visit()
        {
            // Value[0] = x
            // Value[1] = y
            Global.game_map.remove_visit(new Vector2(process_number(command.Value[0]), process_number(command.Value[1])));
            Index++;
            return true;
        }
        ...
        private void command_shop_inventory(Shop_Data shop)
        {
            while (Index + 1 < event_data.data.Count && event_data.data[Index + 1].Key == 85)
            {
                ...
                {
                    string[] item_str = str.Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries);
                    return new Item_Data(process_number(item_str[0]), process_number(item_str[1]), process_number(item_str[2]));
                }));
            }
        }
        ...
        private bool command_add_seize()
        {
            ...
            // Value[2] = y
            Vector2 loc = new Vector2(process_number(command.Value[1]), process_number(command.Value[2]));
            Global.game_map.add_seize_point(process_number(command.Value[0]), loc);
            Index++;
            return true;
        }
        ...
        private bool command_set_variable()
        {
            ...
            switch (command.Value[1])
            {
                ...
                case "UnitX":
                    // Value[2] = Unit ID
                    int unitX = process_unit_id(command.Value[2]);
                    VARIABLES[id] = Convert.ToInt32(Global.game_map.units[unitX].loc.X);
                    break;

                case "UnitY":
                    // Value[2] = Unit ID
                    int unitY = process_unit_id(command.Value[2]);
                    VARIABLES[id] = Convert.ToInt32(Global.game_map.units[unitY].loc.Y);
                    break;
        ...
        private bool command_play_bgm()
        {
            ...
            if (command.Value[0] == "Turn Theme")
            {
                ...
                Global.Audio.play_bgm(command.Value[0]);
            Index++;
            return true;
        }
        ...
        private bool command_fade_bgm()
        {
            ...
            if (frames <= 0)
                Global.Audio.stop_bgm();
            else
                Global.Audio.bgm_fade(frames);
            Index++;
            return true;
        }
        ...
        private bool command_play_sfx()
        {
            // Value[0] Sound group
            // Value[1] = SFX name
            if (command.Value[0] == "System Sounds")
                Global.game_system.play_system_se(command.Value[1]);
            else
                Global.Audio.play_se(command.Value[0], command.Value[1]);
            Index++;
            return true;
        }
        ...
        private bool if_statement_switch(string[] if_key)
        {
            ...
            switch (if_key[0])
            {
                ...
                case "Tier":
                    ...
                    {
                        id = process_number(command.Value[2]);
                        if (Global.game_actors.ContainsKey(id))
                            actor = Global.game_actors[id];
                    }
                    if (actor != null)
                ...
                case "Unit Gender":
                    ...
                    {
                        if (simplify_gender)
                            result = Global.game_map.units[id].actor.gender % 2 == process_number(command.Value[2]) % 2;
                        else
                            result = Global.game_map.units[id].actor.gender == process_number(command.Value[2]);
                    }
                    break;
                case "Visitor Team":
                ...
                case "Unit at Loc":
                    ...
                    break;
                case "Any at Loc": //wark duplicate dupe thing lol
                    unit = Global.game_map.get_unit(new Vector2(process_number(command.Value[1]), process_number(command.Value[2])));
                    result = unit != null;
                    break;
                case "Unit Area":
                ...
                case "Battalion Size":
                    ...
                    {
                        case "<":
                            result = Global.battalion.actors.Count < size;
                            break;
                        case "<=":
                            result = Global.battalion.actors.Count <= size;
                            break;
...
#if DEBUG
                            Print.message(
                                "Invalid operator for \"Battalion Size\" if statement.\n" +
                                "Valid operators are [<, >, <=, >=, ==, !=]");
#endif
                            result = false;
                            break;
                ...
                case "Dead":
                    ...
                    break;
                case "Duping":
                    int unit_idd = Global.game_actors.get_unit_from_actor(3);
                    result = Global.game_map.units[unit_idd].actor.states.Contains(21);
                    break;
                case "Wolframing":
                    int unit_iddd = Global.game_actors.get_unit_from_actor(10);
                    result = Global.game_map.units[unit_iddd].actor.states.Contains(33);
                    break;
                case "Horsieing":
                    int unit_idddd = Global.game_actors.get_unit_from_actor(6);
                    result = Global.game_system.SWITCHES[41];//Global.game_map.units[unit_idddd].actor.states.Contains(34);
                    break;
                case "Actor Class":
                    int actor_id = process_number(command.Value[1]);
                    if (Global.game_actors.ContainsKey(actor_id))
                        result = Global.game_actors[actor_id].class_id == process_number(command.Value[2]);
                    break;
                #endregion
                #region Deployment
                ...
                case "Any Deployment Saved":
                    result = Global.battalion.deployed.Count > 0;
                    break;
                case "PC Deployment Saved":
                    // Value[1] = actor id
                    result = Global.battalion.deployed.Contains(process_number(command.Value[1]));
                    break;
                #endregion
                #region Visiting
                case "Seized":
                ...
                case "Team Size":
                    ...
                    {
                        case "<":
                            result = Global.game_map.teams[team_id].Count < size;
                            break;
                        case "<=":
                            result = Global.game_map.teams[team_id].Count <= size;
                            break;
...
#if DEBUG
                        Print.message(
                            "Invalid operator for \"Team Size\" if statement.\n" +
                            "Valid operators are [<, >, <=, >=, ==, !=]");
#endif
                            result = false;
                            break;
                ...
                case "Gold":
                    //value
                    result = (Global.game_battalions.battalion.gold >= process_number(command.Value[1]));
                    break;
                case "Loss":
