...
namespace FEGame
{
    public class Game1 : Game
    {
        ...
        readonly static Version OLDEST_ALLOWED_SUSPEND_VERSION = new Version(0, 5, 5, 5);
 ...
 DepthFormat.None,
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            Vector2 render_size = new Vector2(ShaderRenderTargets[0].Width,
        ...
        private bool update_zoom()
        {
            ...
            if (Global.zoom != ZOOM || Global.fullscreen != fullscreen)//fullscreen test // graphics.IsFullScreen)
            {
                refresh_zoom();
        ...
        private void refresh_zoom(bool initial_set)
        {
            ZOOM = Global.zoom;
            // Resize window
            if (fullscreen != Global.fullscreen) //fullscreen test
            {
        ...
        private void handle_frame_timing(GameTime game_time)
        {
            ...
            if (Process_Time < (1f / FRAME_RATE))
            {
                if (!IsFixedTimeStep)
                {
                    var sleep_time = TimeSpan.FromTicks((long)(TimeSpan.TicksPerSecond * ((1f / FRAME_RATE) - Process_Time)));
            ...
            while (true)
            {
                ...
                else if (Global.delete_file)
                {
                    ...
                    if (device != null && device.IsConnected)
                    {
                        ...
                        if (Global.suspend_file_info != null && Global.suspend_file_info.save_id == Global.start_game_file_id)
                ...
                else if (Global.delete_map_save)
                {
                    ...
                    if (device != null && device.IsConnected)
                    {
                        ...
                        delete_file(FILE_ID + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION);
                        delete_file(FILE_ID + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION);
        ...
        private void update_suspend()
        {
            if (Global.scene.suspend_calling && !Global.scene.suspend_blocked())
            {
                if (device != null && device.IsConnected)
                {
                    ...
                    Global.scene.suspend_calling = false;
                    Global.scene.reset_suspend_filename();
            ...
            KeyboardState key_state, GamePadState controller_state)
        {
            ...
            if (pause_enabled())
            {
                ...
                if ((pause_pressed && !Pause_Pressed) || (!Paused && (next_pressed && !Next_Pressed)))
                    Paused = !Paused;
                process_next_frame = !Paused || (next_pressed && !Next_Pressed);
            }
        ...
        {
            return Paused || (PAUSE_ENABLED && Global.scene.scene_type != "Scene_Map_Unit_Editor");
        }
        #endregion
        ...
        {
            Thread.Sleep(10);
            Tuple<Version, DateTime> result = null;
#if WINDOWS || MONOMAC
        ...
        private void change_scene(string new_scene)
        {
            ...
            switch (new_scene)
            {
                ...
                case "Scene_Save":
                    ...
                    break;
#if !MONOGAME && DEBUG
                case "Scene_Worldmap_Unit_Editor":
                    Global.game_temp = new Game_Temp();
                    Global.game_battalions = new Game_Battalions();
                    Global.new_game_actors();

                    Global.change_to_new_scene("Scene_Worldmap_Unit_Editor");
                    break;
#endif
                case "Start_Chapter":
                    end_move_range_thread();
        ...
        private void draw_to_screen(SpriteBatch sprite_batch, RenderTarget2D render_target)
        {
            ...
            update_screenshot(render_target);
        }
        ...
        private void update_screenshot(RenderTarget2D render_target)
        {
            ...
            if (f12_pressed && !F12_Pressed)
            {
                ...
                int i = 0;
                while (File.Exists(path + filename + i.ToString("D4") + ".png"))
                    i++;
                using (FileStream fs = new FileStream(path + filename + i.ToString("D4") + ".png", FileMode.OpenOrCreate))
                {
        ...
        protected string convert_save_name()
        {
            ...
            // Otherwise load the suspend for the current file
                ...
                return FILE_ID + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION;
        }
                ...
                else
                {
                    using (Stream stream = container.OpenFile(filename, FileMode.Open))
                    {
                        try
                        {
                            using (BinaryReader reader = new BinaryReader(stream))
                                Global.LOADED_VERSION = load_version(reader);
                            if (Global.LOADED_VERSION.older_than(OLDEST_ALLOWED_SAVE_VERSION))
                                return false;
                        }
                        catch (EndOfStreamException e)
                        {
        ...
        private bool load_file(Stream stream, out Version v, out Save_File_Data data)
        {
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                if (v.older_than(OLDEST_ALLOWED_SAVE_VERSION))
        ...
        protected bool move_file(int id, int move_to_id, bool copying = false)
        {
            ...
            using (StorageContainer container = device.EndOpenContainer(result))
            {
                ...
                //if (container.FileExists(id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION))
                //    container.DeleteFile(id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION);

                // Copy old file to new location
                using (Stream move_from_stream = container.OpenFile(filename, FileMode.Open))
        ...
        {
            string source_suspend_filename = id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION;
            string target_suspend_filename = move_to_id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION;

            copy_suspend(id, move_to_id, container, source_suspend_filename, target_suspend_filename);
        }
        private void copy_suspend(int id, int move_to_id, StorageContainer container, string source_suspend_filename, string target_suspend_filename)
        {
            if (container.FileExists(source_suspend_filename))
                using (Stream move_from_stream = container.OpenFile(source_suspend_filename, FileMode.Open))
                {
        ...
        {
            string source_suspend_filename = id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION;
            string target_suspend_filename = move_to_id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION;
        ...
        protected bool delete_save_file(int id, StorageContainer container)
        {
            ...
            if (container.FileExists(id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION))
                container.DeleteFile(id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION);
            if (container.FileExists(id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION))
                container.DeleteFile(id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION);
            ...
            using (StorageContainer container = device.EndOpenContainer(result))
            {
                ...
                if (container != null)
                {
                    ...
                    using (Stream stream = container.CreateFile(filename))
                    {
                        ...
                        using (BinaryWriter writer = new BinaryWriter(stream))
                        {
                            Global.save_suspend(writer, FILE_ID);
                        }
                        /* Close the Stream */
                        // Close the file.
                    }
        ...
        protected void load_suspend_info(bool update_file_id = true)
        {
            ...
            using (StorageContainer container = device.EndOpenContainer(result))
            {
                ...
                for (i = 0; i < suspend_files.Count; i++)
                {
                    try
                    {
                        using (Stream stream = container.OpenFile(suspend_files[i], FileMode.Open))
                        using (BinaryReader reader = new BinaryReader(stream))
                        {
                            Global.LOADED_VERSION = load_version(reader);
                            if (!Global.LOADED_VERSION.older_than(OLDEST_ALLOWED_SUSPEND_VERSION))
        ...
        protected void load_save_info()
        {
            ...
            using (StorageContainer container = device.EndOpenContainer(result))
            {
                ...
                while (i < save_files.Count)
                {
                    using (Stream stream = container.OpenFile(save_files[i], FileMode.Open))
                    using (BinaryReader reader = new BinaryReader(stream))
                    {
                        ...
                        {
                            if (Global.LOADED_VERSION.older_than(OLDEST_ALLOWED_SAVE_VERSION))
                                throw new EndOfStreamException("Save file is too outdated");
                            ...
                            bool suspend_exists = container.FileExists(file_id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION);
                            bool map_save_exists = container.FileExists(file_id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION);
                            ...
                            if (map_save_exists)
                            {
                                ...
                                using (Stream suspend_stream = container.OpenFile(file_id + Config.MAP_SAVE_FILENAME + Config.SAVE_FILE_EXTENSION, FileMode.Open))
                                {
                                    try
                                    {
                            ...
                            {
                                Version v = Global.LOADED_VERSION;
                                using (Stream suspend_stream = container.OpenFile(file_id + SUSPEND_FILENAME + Config.SAVE_FILE_EXTENSION, FileMode.Open))
                                {
                                    try
                                    {
                                        using (BinaryReader suspend_reader = new BinaryReader(suspend_stream))
